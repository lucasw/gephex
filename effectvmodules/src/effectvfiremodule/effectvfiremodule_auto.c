...
const char* getSpec(void) {
 return "mod_spec { name=[mod_effectvfiremodule] number_of_inputs=[3] number_of_outputs=[1] deterministic=[false] }";
}
...
{
  static const char* INFO = "info { name=[Fire] group=[EffecTV] inputs=[3 Image Background{widget_type=[radio_button] } Mode{widget_type=[combo_box] values=[foreground,light,dark] hidden=[true] help=[burn foreground, light part or dark part] } ] outputs=[1 Image ] type=xpm } ";
  char* tmpBuf;
  ...
  if (buf != 0 && reqLen <= bufLen)
    {
      ...
      for (i = 0; i < lines; ++i)
        {
          char* source = effectvfiremodule_xpm[i];
