...
static int joystick_read_midi(struct Joystick* self, struct OldValues* old,
...
int main(int argc, const char* argv[])
{
  ...
  num_joysticks = joystick_get_num_devices();

  printf("Number of joysticks: %i\n", num_joysticks);

  printf("Opening joystick %i:\n", id);
  jst = joystick_open(id);
  if (jst == 0)
    {
...
static int joystick_read_midi(struct Joystick* jst, struct OldValues* old,
			      unsigned char* midi_buffer,
			      int size, int control_offset)
{  
	int num_axes = joystick_get_num_axes(jst);
	int num_buttons = joystick_get_num_axes(jst);
	int i, len = 0;
