...
public:
  ...
  virtual JoystickDriverImpl *driver() const { return m_driver; }

  int num_axes() const { return m_num_axes; }

  int num_buttons() const { return m_num_buttons; }
...
/ ----------------------------------------------------------------
  ...
  hr = g_pDI->EnumDevices(DI8DEVCLASS_GAMECTRL, EnumJoysticksCallback, NULL,
                          DIEDFL_ATTACHEDONLY) if (FAILED(hr)) return false;
  ...
  hr = joystick->SetCooperativeLevel(hDlg, DISCL_EXCLUSIVE | DISCL_FOREGROUND);
  if (FAILED(hr))
    return false;
  ...
  hr = joystick->EnumObjects(EnumObjectsCallback, (VOID *)hDlg, DIDFT_ALL);
  if (FAILED(hr))
    return false;
