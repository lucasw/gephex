...
void Timer::start() { start_cycles = getTime(); }
...
void Timer::print() {
  cout << "done " << runs << " runs with " << cycles << " cycles at "
       << get_avg_cycles() << " cycles per run (" << get_fps() << " fps)\n";
}

long Timer::get_avg_cycles() { return (long)(cycles / runs); }

double Timer::get_fps() {
  return (double)(cpu_mhz * 1000000) / (cycles / runs);
}

int Timer::get_runs() { return runs; }
