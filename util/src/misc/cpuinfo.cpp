...
bool CpuInfo::check_cpuid() {
  ...
  asm("
      pushfl;
      popl % % eax; movl % % eax, % % ebx; xorl $0x00200000, % % eax;
      pushl % % eax; popfl; pushfl; popl % % eax; cmpl % % eax, % % ebx; jz 0f;
      mov $1, % % eax; mov % % eax, % 0; 0
      :;
      "
      : "=g"(t)::"eax", "ebx", "memory");
#endif
...
void CpuInfo::check_vendor(char *c) {
  ...
  asm("
      cpuid;
      movl % 1, % % eax; movl % % ebx, (% % eax); movl % % edx, 4(% % eax);
      movl % % ecx, 8(% % eax); "
      :
      : "a"(0), "g"(c)
      : "ebx", "ecx", "edx");
...
void CpuInfo::check_cpu_name(char *c) {
  ...
  asm("
      movl $0x80000002,
      % % eax;
      cpuid; movl % 1, % % edi; movl % % eax, (% % edi);
      movl % % ebx, 4(% % edi); movl % % ecx, 8(% % edi);
      movl % % edx, 12(% % edi); movl $0x80000003, % % eax; cpuid;
      addl $16, % % edi movl % % eax, (% % edi); movl % % ebx, 4(% % edi);
      movl % % ecx, 8(% % edi); movl % % edx, 12(% % edi);
      movl $0x80000004, % % eax; cpuid;
      addl $32, % % edi movl % % eax, (% % edi); movl % % ebx, 4(% % edi);
      movl % % ecx, 8(% % edi); movl % % edx, 12(% % edi); "
      : "=g"(c)
      : "g"(c)
      : "eax", "ebx", "ecx", "edx", "edi");
...
char *CpuInfo::get_vendor() { return vendor; }

char *CpuInfo::get_cpu_name() { return cpu_name; }
