...
static int send_data(int socket, struct sockaddr *addr,
                     const unsigned char *buffer, int len);

static int joystick_read_midi(Joystick *self, struct OldValues *old,
                              unsigned char *buf, int len, int control_offset);
...
void parse_commandline(int argc, const char *argv[], const char **host_name,
                       int *port, int *id, int *control_offset) {
  // default values:
  *host_name = "localhost";
  *port = 1337;
  *id = 0;
...
int main(int argc, const char *argv[]) {
  ...
  while (quit == 0) {
    ...
    if (len > 0)
      send_data(socket, (struct sockaddr *)&addr, (unsigned char *)buf, len);
...
static int open_socket(int port, int *socket_ptr) {
  ...
  *socket_ptr = socket(AF_INET, SOCK_DGRAM, 0);
  ...
  if (bind(*socket_ptr, (struct sockaddr *)&local_sin, sizeof(local_sin)) ==
      SOCKET_ERROR) {
...
static int joystick_read_midi(Joystick *jst, struct OldValues *old,
                              unsigned char *midi_buffer, int size,
                              int control_offset) {
