 ...
 **/
/*struct GLHardwareSurface*  createGLHardwareSurface(int w, int h, unsigned int
mode)
{
        ...
        if(!WAS_INIT)
        {
                if(!initGlUtil())
                {
                        MessageBox(NULL, "Couldn't create rendering context
bla...", "error", MB_OK); return NULL;
                }
        }
        ...
        struct GLHardwareSurface* createdSurface =
                (struct GLHardwareSurface*)malloc(sizeof(struct
GLHardwareSurface));
    ...
    nformats;
    if ( !wglChoosePixelFormatARB( hdc, iattributes, fattributes, MAX_PFORMATS,
pformat, &nformats ) )
        {
        fprintf( stderr, "pbuffer creation error:  wglChoosePixelFormatARB()
failed.\n" ); return NULL;
        }
        if ( nformats <= 0 )
        {
        fprintf( stderr, "pbuffer creation error:  Couldn't find a suitable
pixel format.\n" ); return NULL;
        }
    format = pformat[0];

        iattributes[0] = 0;
    createdSurface->surface = wglCreatePbufferARB( hdc, format, w, h,
iattributes ); if ( createdSurface->surface == NULL)
        {
...
HRESULT WindowHandler::create(int w, int h, bool popup) {
  ...
  if (!popup) {
    m_hWnd = CreateWindow(classname, progname, WS_POPUP, CW_USEDEFAULT, 0,
    ...
    m_hWnd = CreateWindowEx(WS_EX_TOPMOST, classname, progname,
                            WS_POPUP | WS_CAPTION | WS_THICKFRAME | WS_BORDER |
                                WS_OVERLAPPED | WS_VISIBLE,
                            CW_USEDEFAULT, 0, m_width, m_height, NULL, NULL,
                            hInstance, NULL);
...
LPDIRECT3DDEVICE8 WindowHandler::getDirectXDevice() {
  ...
  if (NULL == m_device) {
    ...
    if (FAILED(m_d3dInterface->GetAdapterDisplayMode(D3DADAPTER_DEFAULT,
                                                     &d3ddm))) {
    ...
    if (FAILED(m_d3dInterface->CreateDevice(
            D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_hWnd,
            D3DCREATE_MIXED_VERTEXPROCESSING, &d3dpp, &m_device))) {
