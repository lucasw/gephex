...
void initTaggers(PortTagger &portTagger, CommandTagger &commandTagger,
                 net::Protocol &protocol, PortDispatcher &portDispatcher,
                 int port);
...
Controller::Controller(const utils::ConfigManager &config_)
    : config(config_), m_finished(false),
      m_port(config.get_int_param("ipc_port")), tagger1(modelControlSender),
      tagger2(rendererControlSender), tagger3(engineControlSender),
      tagger4(modelControlSender), tagger5(modelControlSender),
      tagger7(modelControlSender),

      tagger10(modelControlSender),

      tagger13(modelControlSender), tagger14(modelControlSender),
      tagger15(modelControlSender),

      portTagger(portDispatcher), protocol(portTagger), socket(0),
      netPoller(protocol, connection_down),

      moduleReceiver(tagger1), moduleClassInfoReceiver(tagger2),
      moduleDataReceiver(tagger3), graphDataReceiver(tagger15),
      controlValueReceiver(tagger4), moduleStatisticsReceiver(tagger5),
      graphNameReceiver(tagger7),

      errorReceiver(tagger10),

      modelStatusReceiver(tagger13), rendererStatusReceiver(tagger14),
  ...
  if (ipcType == "inet") {
    ...
    std::string nodePrefix = config.get_string_param("ipc_unix_node_prefix");
...
#if (ENGINE_VERBOSITY > 0)
    std::cout << "create new snap: " << render_graph_id << " " << render_snap_id
              << std::endl;
#endif
    pModel.newControlValueSetWithID(render_graph_id, render_snap_id,
                                    render_snap_id);
  }

  // activate the graph/snapshot combination
  pModel.changeRenderedGraph(render_graph_id, render_snap_id);

  pModel.changeEditGraph(render_graph_id, render_snap_id);
...
bool Controller::finished() const { return m_finished; }
...
void NetPoller::set_socket(net::ISocket *socket) { m_socket = socket; }
