 ...
 */
void assign(TypeInstanceID destination, TypeInstanceID source);

/**
 * Serializes the value of the instance in the buffer
 */
int serialize(TypeInstanceID instance, char *buffer, int bufferLen);

/**
 * Deserializes the buffer in the instance
 */
void deSerialize(const char *buffer, int len, TypeInstanceID instance);
 ...
 * that.
 */
int convertType(TypeInstanceID destination, TypeInstanceID source,
                TypeAttributesInstanceID attributes);
