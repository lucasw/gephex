...
void GraphFileSystem::loadGraph(const std::string &graphID, Graph &destination,
  ...
  if (it == graphNames2fileNames.end())
    throw std::runtime_error("no graph with that name '" + graphID +
...
void GraphFileSystem::deleteGraph(const std::string &name) {
  std::map<std::string, std::string>::iterator it =
      graphNames2fileNames.find(name);
...
GraphFileSystem::IDNameList GraphFileSystem::getNames(const SpecMap &specMap) {
    ...
    try {
      ...
      if (graphfile) {
        ...
        std::map<std::string, std::string>::const_iterator mapit =
            graphNames2fileNames.find(graph.getID());
        ...
        for (ValueSetMap::const_iterator it = graph.getValueSetMap().begin();
          ...
          valueSetNames.push_back(
              std::make_pair(it->first, it->second->getName()));
        }
...
bool GraphFileSystem::graphExists(const std::string &graphName) const {
  std::map<std::string, std::string>::const_iterator it =
      graphNames2fileNames.find(graphName);
