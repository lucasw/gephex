...
void Graph::addModule(const IModuleClassSpec &spec, int moduleID) {
  ...
  for (unsigned int input = 0; input < spec.inputs().size(); ++input) {
    // all snapshots
    for (Graph::ValueSetMap::iterator i = valueSets.begin();
         i != valueSets.end(); ++i) {
...
void Graph::connectModules(int moduleID1, int outputIndex, int moduleID2,
  ...
  ConnectionMap::const_iterator it =
      m_connections.find(std::make_pair(moduleID2, inputIndex));
...
void Graph::disconnectModules(int moduleID, int inputIndex) {
  ConnectionMap::iterator it =
      m_connections.find(std::make_pair(moduleID, inputIndex));
...
void Graph::deleteModule(int moduleID) {
  for (GraphNodeList::iterator i = m_nodes.begin(); i != m_nodes.end(); ++i) {
    if ((*i)->moduleID() == moduleID) // found
    {
      ...
      for (unsigned int input = 0; input < (*i)->spec().inputs().size();
           ++input) {
        // all snapshots
        for (Graph::ValueSetMap::iterator i = valueSets.begin();
             i != valueSets.end(); ++i) {
...
void Graph::setData(int type, const utils::Buffer &buf) { m_data[type] = buf; }

void Graph::unSetData(int type) { m_data.erase(type); }
...
std::list<int> Graph::moduleDataList(int moduleID) const {
  for (GraphNodeList::const_iterator i = m_nodes.begin(); i != m_nodes.end();
       ++i) {
    if ((*i)->moduleID() == moduleID) // found
    {
...
void Graph::setID(const std::string &id_) { m_id = id_; }

std::string Graph::getID() const { return m_id; }

void Graph::setName(const std::string &name_) { m_name = name_; }

std::string Graph::getName() const { return m_name; }

const Graph::ConnectionMap &Graph::connections() const { return m_connections; }

const Graph::ValueSetMap &Graph::getValueSetMap() const { return valueSets; }

const Graph::GraphNodeList &Graph::nodes() const { return m_nodes; }
...
void Graph::newControlValueSet(const std::string &id, const std::string &name) {
...
void Graph::setControlValue(const std::string &id, int nodeID, int intputIndex,
...
void Graph::copyControlValueSet(const std::string &snapID,
  ...
  */
  ...
  for (ControlValueSet::const_iterator it3 = it->second->begin();
    ...
    copySet->setControlValue(it3->first.first, it3->first.second, it3->second);
  }
}
...
const std::map<int, utils::Buffer> &Graph::data() const { return m_data; }
