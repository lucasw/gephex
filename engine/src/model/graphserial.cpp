...
static std::string::const_iterator unescape(std::vector<unsigned char> &v,
...
static gparser::SGraph graph_to_syntax(const Graph &g) {
  ...
  for (Graph::ValueSetMap::const_iterator it = valueSets.begin();
    ...
    for (ControlValueSet::const_iterator valpair = it->second->begin();
      ...
      values.push_back(
          gparser::SValue(gparser::SPlug(moduleID, plugID), value));
    }
...
static void graph_from_syntax(const gparser::SGraph &sg, model::Graph &graph,
  ...
  for (std::list<gparser::SDataItem>::const_iterator di_it =
           sg.dataitems().begin();
       di_it != sg.dataitems().end(); ++di_it) {
  ...
  for (std::list<gparser::SNode>::const_iterator nit = sg.nodes().begin();
    ...
    if (nodummy) {
      SpecMap::const_iterator it = specMap.find(moduleClassName);
      if (it == specMap.end()) {
        std::ostringstream os;
        os << "No such module class: '" << moduleClassName
           << "' - ignoring module";
    ...
    for (std::list<gparser::SDataItem>::const_iterator di_it =
             node.dataitems().begin();
         di_it != node.dataitems().end(); ++di_it) {
      const gparser::SDataItem &di = *di_it;
      int key = string_to_int(di.key());
      utils::Buffer value = string_to_buffer(di.value());
    ...
    if (nodummy) {
      ...
      if (it1 == moduleSpecs.end() || it2 == moduleSpecs.end()) {
        ...
        try {
          ...
          graph.connectModules(moduleID1, outputIndex, moduleID2, inputIndex);
      ...
      if (nodummy && module_valid[moduleID]) {
        ...
        try {
          graph.setControlValue(snap.id(), moduleID,
                                spec->indexFromInputID(plugID), value);
        } catch (std::exception &e) {
          std::ostringstream os;
...
std::istream &deserializeGraph(std::istream &s, Graph &graph,
                               const SpecMap &specMap, utils::ILogger &logger,
                               bool nodummy) {
  std::ostringstream os;
