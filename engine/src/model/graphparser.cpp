...
SPlug::SPlug(const std::string &node_id, const std::string &plug_id)
    : m_node_id(node_id), m_plug_id(plug_id) {}

std::string SPlug::node_id() const { return m_node_id; }

std::string SPlug::plug_id() const { return m_plug_id; }
...
const SPlug &SValue::plug() const { return m_plug; }

const std::string &SValue::value() const { return m_value; }

//-------------------------------------------------------------------

SSnapshot::SSnapshot(const std::string &id, const std::string &name,
                     std::list<SValue> &values)
    : m_id(id), m_name(name), m_values(values) {}

std::string SSnapshot::id() const { return m_id; }

std::string SSnapshot::name() const { return m_name; }

const std::list<SValue> &SSnapshot::values() const { return m_values; }
...
const SPlug &SConnection::from() const { return m_from; }

const SPlug &SConnection::to() const { return m_to; }

//-------------------------------------------------------------------

SDataItem::SDataItem(const std::string &key, std::string &value)
    : m_key(key), m_value(value) {}

std::string SDataItem::key() const { return m_key; }
const std::string &SDataItem::value() const { return m_value; }

//-------------------------------------------------------------------

SNode::SNode(const std::string &id, const std::string &type,
             std::list<SDataItem> &dataitems)
    : m_id(id), m_type(type), m_dataitems(dataitems) {}

std::string SNode::id() const { return m_id; }

std::string SNode::type() const { return m_type; }

const std::list<SDataItem> &SNode::dataitems() const { return m_dataitems; }

//-------------------------------------------------------------------

SGraph::SGraph(const std::string &id, const std::string &name,
               std::list<SDataItem> &dataitems, std::list<SNode> &nodes,
               std::list<SConnection> &connections,
               std::list<SSnapshot> &snapshots)
    : m_id(id), m_name(name), m_dataitems(dataitems), m_nodes(nodes),
      m_connections(connections), m_snapshots(snapshots) {}

std::string SGraph::id() const { return m_id; }

std::string SGraph::name() const { return m_name; }

const std::list<SDataItem> &SGraph::dataitems() const { return m_dataitems; }

const std::list<SNode> &SGraph::nodes() const { return m_nodes; }

const std::list<SConnection> &SGraph::connections() const {
  return m_connections;
}

const std::list<SSnapshot> &SGraph::snapshots() const { return m_snapshots; }
...
utils::xml::Iterator parse_node(utils::xml::Iterator begin,
                                utils::xml::Iterator end, SNode &node) {
  //      std::cout << "parse node\n";
  utils::xml::Iterator i = begin;
  std::string id;
  std::string type;
...
utils::xml::Iterator parse_plug(utils::xml::Iterator begin,
                                utils::xml::Iterator end,
                                const std::string &tag_name, SPlug &plug) {
  utils::xml::Iterator i = begin;
...
utils::xml::Iterator parse_value(utils::xml::Iterator begin,
                                 utils::xml::Iterator end, SValue &value) {
  utils::xml::Iterator i = begin;
  SPlug plug;
  std::string buf;
...
const SGraph parse(const std::string &text) {
  ...
  try {
    ...
    return SGraph(graph_id, graph_name, data, nodes, connections, snapshots);
...
public:
  ...
  void operator()(const SSnapshot &s) {
    m_stream << "<snapshot> <id>" << s.id() << "</id> <name>" << s.name()
             << "</name>\n";
    m_stream << "<values>\n";

    ValueSerializer vs(m_stream);
    std::for_each(s.values().begin(), s.values().end(), vs);
...
public:
  ...
  void operator()(const SNode &n) {
    ...
    std::for_each(n.dataitems().begin(), n.dataitems().end(), ds);
    m_stream << "</data>\n";
...
void serialize(std::ostream &s, const SGraph &g) {
  ...
  s << "<data>\n";
  DataSerializer ds(s);
  std::for_each(g.dataitems().begin(), g.dataitems().end(), ds);
  s << "</data>\n";
