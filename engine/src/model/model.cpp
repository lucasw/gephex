...
void sendGraphDestruction(const Graph &g,
                          ...
                          IModuleDataReceiver &dr, IGraphDataReceiver &gr) {
  ...
  for (Graph::ConnectionMap::const_iterator it = connections.begin();
    ...
    dumbo.modulesDisconnected(it->second->id2(), it->second->inIndex());
  }
...
void sendGraphConstruction(const Graph &g,
  ...
  if (gr) {
    const std::map<int, utils::Buffer> &data = g.data();
    for (std::map<int, utils::Buffer>::const_iterator lt = data.begin();
         lt != data.end(); ++lt) {
  ...
  for (Graph::GraphNodeList::const_iterator jt = nodes.begin();
       jt != nodes.end(); ++jt) {
    if (dumbo)
      dumbo->moduleAdded((*jt)->spec().moduleClassName(), (*jt)->moduleID());

    if (smartAss)
      smartAss->moduleAdded(graphID, (*jt)->spec().moduleClassName(),
                            (*jt)->moduleID());
    ...
    if (dr) {
      ...
      for (std::map<int, utils::Buffer>::const_iterator lt = datas.begin();
           lt != datas.end(); ++lt) {
        dr->moduleDataSet((*jt)->moduleID(), lt->first, lt->second);
      }
    }
  }
  ...
  while (kt != connections.end()) {
    ...
    if (smartAss)
      smartAss->modulesConnected(graphID, kt->second->id1(),
                                 kt->second->outIndex(), kt->second->id2(),
                                 kt->second->inIndex());
    ++kt;
  }
}
...
bool graphExists(const std::string &graphID, const Model::GraphMap &graphs,
...
void sendSnapShotNames(const std::string &graphID, const std::string &graphName,
                       const Graph::ValueSetMap &sets,
                       IGraphNameReceiver *gnr) {
  for (Graph::ValueSetMap::const_iterator it = sets.begin(); it != sets.end();
       ++it) {
...
void Model::updateFileSystem() {
  ...
  for (namesVector::const_iterator it = namesV.begin(); it != namesV.end();
       ++it) {
    ...
    if (isDirectoryName(graphName)) {
      ...
      for (std::list<std::pair<std::string, std::string>>::const_iterator snap =
               it->second.begin();
           snap != it->second.end(); ++snap) {
        if (gnr != 0)
...
void Model::addModule(const std::string &moduleClassName) {
#if (ENGINE_VERBOSITY > 0)
  std::cout << "Model::addModule: "
            << "moduleClassName: " << moduleClassName << std::endl;
#endif
  ...
  if (renderedGraph == editGraph) {
    for (unsigned int input = 0; input < spec->inputs().size(); ++input) {
      ...
      rendererControlReceiver->setInputValue(renderedGraph->getID(), moduleID,
                                             input, spec->defaultValue(input));
    }
  }
...
void Model::connectModules(int moduleID1, int outputIndex, int moduleID2,
                           int inputIndex) {
  ...
  smartAss->modulesConnected(editGraph->getID(), moduleID1, outputIndex,
...
void Model::setModuleData(int moduleID, int type, const utils::Buffer &buf) {
  ...
  std::cout << "setModuleData: modid=" << moduleID << ", type = " << type << " "
            << content << std::endl;
  delete[] content;
#endif
...
#if (ENGINE_VERBOSITY > 1)
  std::cout << "unSetModuleData: modid=" << moduleID << ", type = " << type
            << std::endl;
#endif
...
void Model::setEditGraphData(int type, const utils::Buffer &buf) {
  ...
  memcpy(content, buf.getPtr(), buf.getLen());
  content[buf.getLen()] = 0;
  std::cout << "setEditGraphData: type=" << type << " " << content << std::endl;
  delete[] content;
#endif
...
bool Model::check_for_graph_id(const std::string &graph_id) {
  ...
  GraphMap::const_iterator it =
      lookForGraph(graph_id, graphs, specs, *fileSystem, smartAss, m_logger);
...
void Model::newGraphWithID(const std::string &graphName,
                           const std::string &graphID, bool notifyAndCreate) {
#if (ENGINE_VERBOSITY > 1)
  ...
  if (graphs.end() !=
      std::find_if(graphs.begin(), graphs.end(), has_name(graphName)))
...
void Model::deserializeGraph(const std::string &graphstream) {
  ...
  if (it == graphs.end()) { // this name doesn't exist
...
void Model::moduleClassUnloaded(const std::string &moduleClassName) {
  ...
  if (it == specs.end())
    throw std::runtime_error(
        "moduleclasse doesnt exist (Model::moduleClassUnloaded)");
...
void Model::controlValueChanged(const std::string &graphID, int nodeID,
                                int inputIndex,
                                const utils::Buffer &newValue) // Renderer
{
  ...
  if (it == graphs.end()) {
    ...
    m_logger->error("Model", "graphID doesnt exist (controlValueChanged)");
    return;
  }
  ...
  if (editGraph == renderedGraph && editControlSet == renderedControlSet) {
    controlValueReceiver->controlValueChanged(nodeID, inputIndex, newValue);
  }
...
void Model::registerModuleConstructionDumbReceiver(
    IModuleConstructionDumbReceiver &r) {
  dumbo = &r;
}

void Model::registerModuleConstructionSmartReceiver(
    IModuleConstructionSmartReceiver &r) {
  smartAss = &r;
}
...
void Model::registerGraphNameReceiver(IGraphNameReceiver &r) { gnr = &r; }

void Model::registerModuleDataReceiver(IModuleDataReceiver &r) { dr = &r; }

void Model::registerGraphDataReceiver(IGraphDataReceiver &r) { gr = &r; }
...
void Model::changeRenderedGraph(const std::string &graphID,
  ...
  if (renderedGraph == 0 || graphID != renderedGraph->getID() ||
    ...
    for (ControlValueSet::const_iterator setIt = renderedControlSet->begin();
         setIt != renderedControlSet->end(); ++setIt) {
      int moduleID = setIt->first.first;
      int inputIndex = setIt->first.second;
      try {
        rendererControlReceiver->setInputValue(graphID, moduleID, inputIndex,
                                               setIt->second);
      } catch (std::runtime_error &e) {
        std::ostringstream os;
        os << e.what() << "\n";
        os << "Removing value of module " << moduleID << ", input "
           << inputIndex << " from snapshot.\n";
    ...
    for (InputList::const_iterator it = del_list.begin(); it != del_list.end();
         ++it) {
...
void Model::saveGraph(const std::string &graphID) {
  ...
  if (it == graphs.end())
    throw std::runtime_error("(Model::saveGraph) no graph with id: " + graphID);
  ...
  for (std::string::iterator strit = graphName.begin();
    ...
    if (*strit == '/' && strit != graphName.begin()) {
      ...
      GraphMap::const_iterator dirIt =
          std::find_if(graphs.begin(), graphs.end(), has_name(dir));
      if (dirIt != graphs.end()) {
        fileSystem->saveGraph(*dirIt->second);
      } else {
        std::ostringstream os;
        os << "graph " << graphName
           << " is in a unknown directory with the name";
        os << " : " << dir;
        m_logger->error("Model", os.str());
      }
    }
...
void Model::deleteGraph(const std::string &graphID) {
  ...
  } else // graph is not loaded
  {
    ...
    if (valueSetList.empty())
      ...
      for (GraphFileSystem::ValueSetList::const_iterator it =
               valueSetList.begin();
...
void Model::changeEditGraph(const std::string &graphID,
  ...
  if (editGraph != it->second || snapID != editControlSet->getID()) {
    ...
    for (ControlValueSet::const_iterator setIt = editControlSet->begin();
         setIt != editControlSet->end(); ++setIt) {
      controlValueReceiver->controlValueChanged(
          setIt->first.first, setIt->first.second, setIt->second);
    }
  }
}
...
void Model::syncInputValue(int moduleID, int inputIndex) {
  ...
  ControlValueSet::const_iterator it =
      editControlSet->find(std::make_pair(moduleID, inputIndex));
  ...
  if (it == editControlSet->end()) {
    ...
    controlValueReceiver->controlValueChanged(moduleID, inputIndex, it->second);
  }
}
...
void Model::modExecTimeSignal(const std::string &graphID, int nodeID,
                              double execTime) {
