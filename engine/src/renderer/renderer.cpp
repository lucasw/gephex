...
public:
  ...
  void controlValueChanged(int moduleID, int inputIndex,
    ...
    m_receiver->controlValueChanged(m_graphID, moduleID, inputIndex, buf);
  }
...
void Renderer::registerSmartControlValueReceiver(
    ISmartControlValueReceiver &r) {
  cvr = &r;
}

void Renderer::registerModuleStatisticsSmartReceiver(
    IModuleStatisticsSmartReceiver &r) {
  msr = &r;
}
...
void Renderer::graphDeleted(const std::string &graphID) {
  ...
  if (it == graphs.end()) {
    m_logger->error("Renderer::graphDeleted",
                    "Graph with id = '" + graphID + "' does not exist.");
    return;
  }
...
#if (ENGINE_VERBOSITY > 0)
  std::cout << "Renderer::moduleAdded: "
            << "graphID:" << graphID << " moduleClassName: " << moduleClassName
...
void Renderer::modulesConnected(const std::string &graphID, int moduleID1,
                                int outputIndex, int moduleID2,
                                int inputIndex) {
#if (ENGINE_VERBOSITY > 0)
  std::cout << "Renderer::modulesConnected: "
            << "graphID" << graphID << " moduleID1: " << moduleID1
...
void Renderer::modulesDisconnected(const std::string &graphID, int moduleID,
                                   int inputIndex) {
...
void Renderer::setInputValue(const std::string &graphID, int nodeID,
                             int inputIndex, const utils::Buffer &buf) {
  ControlValueReceiverWrapper wrapper(cvr, graphID);
  find(graphID)->setInputValue(nodeID, inputIndex, buf, &wrapper);
}

void Renderer::syncInputValue(const std::string &graphID, int nodeID,
                              int inputIndex) {
  ControlValueReceiverWrapper wrapper(cvr, graphID);
