...
CModuleFunctionTable loadModule(utils::SharedLibrary &sl) {
  ...
  ft.strongDependenciesCalculated =
      (strongDependenciesCalculatedT)sl.loadSymbol(
          "strongDependenciesCalculated");

  ft.getInputAttributes =
      (getInputAttributesT)sl.loadSymbol("getInputAttributes");

  ft.getPatchLayout = (getPatchLayoutT)sl.loadSymbol("getPatchLayout");

  // check for missing functions
  if (ft.newInstance == 0 || ft.deleteInstance == 0 || ft.update == 0 ||
      ft.getSpec == 0 || ft.getInfo == 0)
