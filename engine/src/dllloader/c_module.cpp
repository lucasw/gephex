				 ...
				 
{
	...
	for (unsigned int j = 0; j < attributes.outputs.size(); ++j)
    {
		...
		m_vtable->setOutput(m_instance,j,type->getPointer());
    }
...
//TODO: why is type not used?????
//TODO: remove this method
void CModule::setData(const IInput* in/*,const IType* type*/) //TODO
{
	int index = in->getIndex();
	for (std::vector<IInputPtr>::const_iterator it = inputs.begin(); 
	it != inputs.end(); ++it)
    {
		IInputPtr current = *it;
		if (current->getIndex() == index)
		{
			m_vtable->setInput(m_instance,index,in->getData()->getPointer());
			return;
		}
    }
	throw std::runtime_error("Falscher Input bei CModule::setData()");
}
