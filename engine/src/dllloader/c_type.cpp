...
void CType::deSerialize(const utils::Buffer &buf) {
  vTab->deSerialize(reinterpret_cast<const char *>(buf.getPtr()), buf.getLen(),
                    instance);
}

void *CType::getPointer() const { return instance; }

bool CType::equalAttributes(const TypeAttributes &attr) const {
  return vTab->attributesEqual(
      instance, const_cast<void *>(static_cast<const void *>(&attr)));
}
