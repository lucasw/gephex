...
CInput::CInput(int _typeID, bool _const, bool _strong, std::shared_ptr<IModule> cmod, int index,
  ...
  // set input to default value
  {
    ...
    if (m_attr != 0 && !defaultValue.equalAttributes(*m_attr)) { // convert
...
void CInput::unPlug() {
  if (m_isConnected) {
    ...
    if (m_attr != 0 && !connectedData->equalAttributes(
                           *m_attr)) { // input attribs are fixed and differ a
...
int CInput::getType() const { return typeID; }

bool CInput::isConst() const { return _isConst; }

bool CInput::isStrongDependency() const { return _isStrong; }
...
void CInput::setValue(const utils::Buffer &serializedData) {
  if (!m_isConnected) {
    ...
    if (m_attr != 0 && !internalData->equalAttributes(*m_attr)) { // convert
...
void CInput::update() {
  if (m_isConnected) { // input is connected
    assert(oPlug != 0);
    const IType *connectedData = oPlug->getData();
