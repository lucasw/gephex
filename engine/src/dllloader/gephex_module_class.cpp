...
CModuleClass::CModuleClass(const CModuleFunctionTable &ft,
  ...
  for (int i = 0; i < numInputs; ++i) {
    ...
    try {
      ...
      defaultVals[i] = utils::Buffer(
          reinterpret_cast<const unsigned char *>(defaultVal.c_str()),
          defaultVal.length() + 1);
    } catch (...) {
    }
    ...
    std::cout << "inputNames[" << i << "] = \"" << inputNames[i] << '"'
              << std::endl;
#endif
...
const utils::Buffer &CModuleClass::info() const { return m_info; }

const ModuleClassSpec &CModuleClass::spec() const { return *m_spec; }
