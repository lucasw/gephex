...
void f0rwrapper_update(void *instance) {
  ...
  case F0R_PLUGIN_TYPE_SOURCE: {
    attribs.xsize = trim_int(
        reinterpret_cast<NumberType *>(wr->input_params[0])->number, 8, 4096);

    attribs.ysize = trim_int(
        reinterpret_cast<NumberType *>(wr->input_params[1])->number, 8, 4096);
    offset = 2;
    break;
  }
  ...
  case F0R_PLUGIN_TYPE_MIXER3: {
    ...
    attribs.xsize = std::max(in1_attribs.xsize,
                             std::max(in2_attribs.xsize, in3_attribs.xsize));
    attribs.ysize = std::max(in1_attribs.ysize,
                             std::max(in2_attribs.ysize, in3_attribs.ysize));
  ...
  case F0R_PLUGIN_TYPE_SOURCE: {
    if (wr->funs->update != 0) {
      wr->funs->update(wr->f0r_instance, time, 0, wr->outf->framebuffer);
    } else {
      wr->funs->update2(wr->f0r_instance, time, 0, 0, 0, wr->outf->framebuffer);
    }
    break;
  }
  case F0R_PLUGIN_TYPE_FILTER: {
    if (wr->funs->update != 0) {
      wr->funs->update(
          wr->f0r_instance, time,
          reinterpret_cast<FrameBufferType *>(wr->input_params[0])->framebuffer,
          wr->outf->framebuffer);
    } else {
      wr->funs->update2(
          wr->f0r_instance, time,
          reinterpret_cast<FrameBufferType *>(wr->input_params[0])->framebuffer,
          0, 0, wr->outf->framebuffer);
    }
    break;
  }
  case F0R_PLUGIN_TYPE_MIXER2: {
    wr->funs->update2(
        wr->f0r_instance, time,
        reinterpret_cast<FrameBufferType *>(wr->input_params[0])->framebuffer,
        reinterpret_cast<FrameBufferType *>(wr->input_params[1])->framebuffer,
        0, wr->outf->framebuffer);
    break;
  }
  case F0R_PLUGIN_TYPE_MIXER3: {
    wr->funs->update2(
        ...
        reinterpret_cast<FrameBufferType *>(wr->input_params[0])->framebuffer,
        reinterpret_cast<FrameBufferType *>(wr->input_params[1])->framebuffer,
        reinterpret_cast<FrameBufferType *>(wr->input_params[2])->framebuffer,
        wr->outf->framebuffer);
    break;
  }
  default:
...
std::string frei0r_create_spec(const f0r_plugin_info_t &f0r_info) {
  ...
  // return "mod_spec { name=[mod_gradnormmodule] number_of_inputs=[3]
...
frei0r_create_in_param_spec(const f0r_plugin_info_t &f0r_info,
                            const frei0r_funs_t *frei0r, int input,
                            std::vector<f0r_param_info_t> &param_infos) {
  // return "input_spec { type=typ_FrameBufferType id=b const=true
  ...
  case F0R_PLUGIN_TYPE_SOURCE:
    if (input == 0 || input == 1) {
      ...
      os << "typ_NumberType; id=frei0r_" << param_name
         << " const=true strong_dependency=true default=" << default_value
         << "}";
      return os.str();
    }
    index = input - 2;
...
frei0r_create_info(const f0r_plugin_info_t &f0r_info,
  ...
  std::ostringstream os;
  int num_inputs = frei0r_num_inputs(f0r_info);
  //  static const char* INFO = "info { name=[Gradientnorm] group=[Filter]
  //  inputs=[3 Image Lightness{lower_bound=[0] step_size=[0.01]
  //  higher_bound=[1] hidden=[true] help=[lightness] }
  //  Algorithm{widget_type=[combo_box] values=[sobel,roberts-cross]
  //  hidden=[true] help=[sobel is slower and smoother, roberts-cross is quick
  //  and dirty] } ] outputs=[1 Image ] type=xpm } ";

  os << "info { name=[" << f0r_info.name << "] group=[Frei0r] inputs=["
     << num_inputs;
  ...
  for (int i = 0; i < f0r_info.num_params; ++i) {
    ...
    case F0R_PARAM_BOOL:
      properties = "{widget_type=radio_button}";
      break;
    case F0R_PARAM_DOUBLE:
      properties = "{widget_type=number_selector}";
      break;
    default:
      properties = "";
    }
...
namespace {
// Default functions
int Dinit(void (*)(int, const char *, const char *)) { return 1; }
...
void *DgetInputAttributes(int) { return 0; }
} // namespace
...
Frei0rModuleClass::Frei0rModuleClass(const frei0r_funs_t &frei0r,
  ...
  if (m_functionTable.newInstance == 0 || m_functionTable.deleteInstance == 0 ||
  ...
  std::vector<utils::Buffer> defaultVals(
      numInputs, utils::Buffer(reinterpret_cast<const unsigned char *>(""), 1));
  ...
  for (int i = 0; i < numInputs; ++i) {
    ...
    std::string param_name;
    inSpec =
        frei0r_create_in_param_spec(f0r_info, &m_frei0r, i, f0r_param_infos);
...
const utils::Buffer &Frei0rModuleClass::info() const { return m_info; }

const ModuleClassSpec &Frei0rModuleClass::spec() const { return *m_spec; }
...

{
  ...
  instance = harr(&m_frei0r, m_defaultInputValues.size(), m_frei0r_plugin_type);
