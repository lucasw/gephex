 ...
 */
class DllLoader : public IModuleClassInfoSender,
...
public:
  ...
  DllLoader(utils::AutoPtr<utils::ILogger> &logger,
            ...
            const std::string &module_path, const std::string &type_path,
  ...
  void registerModuleClassReceiver(IModuleClassReceiver &r) { assert(false); }
  void registerTypeClassReceiver(ITypeClassReceiver &) { assert(false); }
