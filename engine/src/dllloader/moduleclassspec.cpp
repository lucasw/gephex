...
ModuleClassSpec::ModuleClassSpec(const std::string& mClassName, const std::vector<int>& ins,
								 const std::vector<utils::Buffer>& defaultValues,
								 const std::vector<int>& outs)
 : m_moduleClassName(mClassName), m_inputs(ins), m_outputs(outs), m_defaultValues(defaultValues)
{
}

ModuleClassSpec::ModuleClassSpec(const ModuleClassSpec& spec)
 : IModuleClassSpec(),m_moduleClassName(spec.m_moduleClassName), m_inputs(spec.m_inputs),
	m_outputs(spec.m_outputs), m_defaultValues(spec.m_defaultValues)
{
}
