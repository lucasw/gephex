...
ModuleClassSpec::ModuleClassSpec(const std::string &mClassName,
    ...
    : m_moduleClassName(mClassName), m_inputs(ins),
      m_defaultValues(defaultVals), m_inIDs(inIDs), m_outputs(outs),
      m_outIDs(outIDs) {}

ModuleClassSpec::ModuleClassSpec(const ModuleClassSpec &spec)
    : IModuleClassSpec(), m_moduleClassName(spec.m_moduleClassName),
      m_inputs(spec.m_inputs), m_defaultValues(spec.m_defaultValues),
      m_inIDs(spec.m_inIDs), m_outputs(spec.m_outputs),
      m_outIDs(spec.m_outIDs) {}

// std::string identifier() const;
const std::vector<int> &ModuleClassSpec::inputs() const { return m_inputs; }

const std::vector<int> &ModuleClassSpec::outputs() const { return m_outputs; }
