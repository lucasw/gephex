...
LoadModuleTest::LoadModuleTest() : EasyTest("LoadModuleTest", "") {}
...
void DummyModuleClassSpecReceiver::moduleClassLoaded(
    int moduleClassID, const ModuleClassSpec &specco) {
  const std::vector<int> &ins = specco.inputs();
  const std::vector<int> &outs = specco.outputs();
  int moduleClassID_ = specco.moduleClassID();

  if (ins.size() != 2)
    throw std::runtime_error("Anzahl inputs nicht zwei!!!");
  if (ins[0] != ins[1])
    throw std::runtime_error("Inputs sind nicht gleich!!!");
  if (outs.size() != 1)
    throw std::runtime_error("Anzahl outputs nicht eins!!!");
  if (ins[0] != outs[0])
    throw std::runtime_error("ins[0] != outs[0]");
  if (moduleClassID_ != moduleClassID)
    throw std::runtime_error(
        "Übergebene modClassID ungleich der, die im spec steht...");
}
...
void DummyModuleClassInfoReceiver::moduleClassLoaded(int moduleClassID,
                                                     const ModuleInfo &mi) {
  const char *soll =
      " { group=[Shit]; gui_name=[Adder]; identifier=[mod_addNumbers]; "
      "inputs=[2 Summand1:typ_number Summand2:typ_number ]; outputs=[1 "
      "Summe:typ_number ]; type=[xpm];  } ";
...
public:
  virtual void moduleClassLoaded(int moduleClassID,
