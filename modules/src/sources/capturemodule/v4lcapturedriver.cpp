...
void cvt_bgra32_to_bgra(uint8_t *dst_, uint8_t *src_, size_t width,
                        size_t height) {
...
public:
  ...
  void open(int device_num) {
    ...
    try {
      ...
      for (int i = 0; i != 2; ++i) {
        m_frame_ptr.push_back(reinterpret_cast<uint32_t *>(
            reinterpret_cast<char *>(m_mmapBase) + videoMBuf.offsets[i]));
      }
  ...
  bool is_open() const { return vd.get() != 0; }
...
void V4LCaptureDriver::decode_frame(uint_32 *framebuffer, int width,
                                    int height) {
  if (!m_impl->is_open())
...
void V4LCaptureDriver::status(int &width, int &height, bool &new_sample) const {
  width = m_impl->width();
  height = m_impl->height();
  new_sample = m_impl->new_sample();
}

bool V4LCaptureDriver::is_open() const { return m_impl->is_open(); }
