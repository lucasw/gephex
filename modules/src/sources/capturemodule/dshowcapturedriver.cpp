...
IBaseFilter *get_capture_device(int device_num) {
  ...
  CoCreateInstance(CLSID_SystemDeviceEnum, 0, CLSCTX_INPROC_SERVER,
                   IID_ICreateDevEnum, (void **)&createDeviceEnum);
...
static BOOL GetFormattedError(LPTSTR dest, int size) {
  ...
  return 0 != FormatMessage(flags, 0, dwLastError,
...
public:
  Impl()
      : /*pEvent(0),*/ m_grabber_base_filter(0), m_graph(0), m_media_control(0),
        m_sample_grabber(0), m_capture_filter(0), m_width(0), m_height(0) {
    CoInitialize(0);
  ...
  void buildFilterGraph(int device_num) {
    ...
    build_grabber_graph(&m_graph, &m_media_control, &m_grabber_base_filter,
                        &m_sample_grabber, 0, &m_callback);
    ...
    connect_source_filter(m_graph, m_grabber_base_filter, m_sample_grabber, 0,
                          m_capture_filter, m_width, m_height);
  }
  ...
  void grab_frame(uint_32 *frb, int width, int height) {
    ...
    ls_scale32m(frb, width, height, reinterpret_cast<uint_32 *>(samples),
...
DSHOWCaptureDriver::DSHOWCaptureDriver() : m_impl(new Impl()) {}
...
void DSHOWCaptureDriver::decode_frame(uint_32 *framebuffer, int width,
                                      int height) {
  if (!m_impl->is_open())
...
void DSHOWCaptureDriver::status(int &width, int &height,
...
bool DSHOWCaptureDriver::is_open() const { return m_impl->is_open(); }
