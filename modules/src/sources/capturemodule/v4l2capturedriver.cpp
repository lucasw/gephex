...
static void cvt_bgra32_to_bgra(uint8_t *dst_, uint8_t *src_, size_t width,
                               size_t height) {
...
static void init_device(int fd, unsigned int &width, unsigned int &height,
...
public:
  Impl()
      : m_fd(-1), m_width(0), m_height(0), m_buffers(0),
        m_capturing_started(false), m_tmp_image(0), m_tmp_image_size(0) {}
  ~Impl() {}
  ...
  void grab_frame(uint_32 *frb, int width, int height) {
    if (m_fd == -1 || !m_buffers || width != m_width || height != m_height) {
      ...
      m_fd =
          ::open(m_device_name.c_str(), O_RDWR /* required */ | O_NONBLOCK, 0);
      if (m_fd == -1)
    ...
    case V4L2_PIX_FMT_BGR32: // tested with a bttv 878 card
      cvt_bgra32_to_bgra(reinterpret_cast<uint_8 *>(m_tmp_image),
                         reinterpret_cast<uint_8 *>(raw_img), m_width,
                         m_height);
      break;
...
V4L2CaptureDriver::V4L2CaptureDriver() : m_impl(new Impl()) {}
...
void V4L2CaptureDriver::decode_frame(uint_32 *framebuffer, int width,
                                     int height) {
  if (!m_impl->is_open())
...
void V4L2CaptureDriver::status(int &width, int &height,
                               bool &new_sample) const {
  width = m_impl->width();
...
bool V4L2CaptureDriver::is_open() const { return m_impl->is_open(); }
