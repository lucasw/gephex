...
struct device_descriptor {
  device_descriptor() : drv(0), dev_num(-1), ref_count(0) {}
...
void update(void *instance) {
  ...
  if (my->device_num != device_num || my->drv == 0 || !my->drv->is_open()) {
    try {
      if (my->device_num != -1) {
        my->drv = 0;
        close_device(my->device_num);
  ...
  try {
    ...
    my->drv->decode_frame(inst->out_result->framebuffer, xsize, ysize);
  } catch (std::exception &e) {
    s_log(0, e.what());
