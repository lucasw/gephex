...
#define VIDIOCSTUNER                                                           \
  _IOW('v', 5, struct video_tuner) /* Tune the tuner for the current channel   \
                                    */
#define VIDIOCGPICT                                                            \
  _IOR('v', 6, struct video_picture) /* Get picture properties */
#define VIDIOCSPICT                                                            \
  _IOW('v', 7, struct video_picture)    /* Set picture properties */
#define VIDIOCCAPTURE _IOW('v', 8, int) /* Start, end capture */
#define VIDIOCGWIN                                                             \
  _IOR('v', 9, struct video_window) /* Get the video overlay window */
#define VIDIOCSWIN                                                             \
  _IOW('v', 10,                                                                \
       struct video_window) /* Set the video overlay window - passes clip list \
                               for hardware smarts , chromakey etc */
#define VIDIOCGFBUF _IOR('v', 11, struct video_buffer) /* Get frame buffer */
#define VIDIOCSFBUF                                                            \
  _IOW('v', 12, struct video_buffer) /* Set frame buffer - root only */
#define VIDIOCKEY                                                              \
  _IOR('v', 13, struct video_key) /* Video key event - to dev 255 is to all -  \
...
#define VIDIOCGPLAYINFO                                                        \
  _IOR('v', 26, struct video_info) /* Get current playback info from hardware  \
                                    */
