...
static int my_max(int a, int b) { return (a < b) ? b : a; }
...
static void info_print(const struct stream_info *info) {
  printf("%s WXH = %ix%i, fps = %f, I = %c, A = %f\n",
         info->color_model == YUV420P ? "YUV420P" : "BGRA8888", info->width,
         info->height, info->fps, info->c, info->A);
}
...
static void queue_flush(struct res_queue *q) { q->buf_len = 0; }
...
static void close_file(int file_handle) { close(file_handle); }
...
static int to_int(char c) { return (int)(c - '0'); }
...
static int scan_named_rational(const uint8_t *data, int len, const char *name,
                               double *val) {
  int retval;
  int nom, denom;
  const uint8_t *ptr = data;
...
static int scan_data(uint8_t *data, int len, struct stream_info *info,
                     struct scan_status *status, int xsize, int ysize,
                     FrameBufferType *scaled_fb, FrameBufferType *out) {
  int retval;
  int bytes_left = len;
  uint8_t *ptr = data;
  int framesize;
  ...
  if (bytes_left >= framesize) // whole frame available
  {
    ...
    if (info->color_model == YUV420P)
      cvt_yuv420p_to_bgra((uint8_t *)target, ptr, info->width, info->height);
    else
      memcpy(target, ptr,
             get_frame_size(info->width, info->height, info->color_model));
...
void update(void *instance) {
  ...
  if (new_data) {
    ...
    if (my->info.width == 0 || my->info.height == 0 ||
      ...
      read_size = get_frame_size(my->info.width, my->info.height,
                                 my->info.color_model) +
                  6 - my->queue.buf_len;
    ...
    read_size =
        read(my->file_handle, my->queue.buf + my->queue.buf_len, read_size);
  ...
  if (my->queue.buf_len != 0) {
    // scan the queue
    scanned_bytes =
        scan_data(my->queue.buf, my->queue.buf_len, &my->info, &my->status,
                  xsize, ysize, &my->scaled_fb, inst->out_result);
