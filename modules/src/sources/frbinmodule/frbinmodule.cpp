...
int init(logT log_function) {
  s_log = log_function;
  char buffer[128];
  snprintf(buffer, sizeof(buffer), "setting up cache: %i mb", CACHE_SIZE_IN_MB);
  s_log(2, buffer);
  ...
  //  DriverConstructor<DSHOWDriver>* DSHOW_ctor = new
  //  DriverConstructor<DSHOWDriver>();
...
static std::string get_extension(const std::string &file_name) {
  ...
  if (pos == std::string::npos)
    ...
    std::transform<I, I, int (*)(int)>(extension.begin(), extension.end(),
...
void update(void *instance) {
  ...
  try {
    const unsigned int user_xsize = trim_int(inst->in_x_size->number, 0, 1920);
    const unsigned int user_ysize = trim_int(inst->in_y_size->number, 0, 1080);
    ...
    const unsigned int frame_number =
        trim_int(inst->in_frame->number, 0, INT_MAX);
      ...
      if (use_cache) {
        ...
        if (it != m_known_infos.end()) {
          ...
          if (my->info.num_frames < 1) {
            ...
            my->damaged = false;
            m_known_infos.insert(std::make_pair(newFileName, my->info));
          }
        }
    ...
    if (!use_cache) {
      // decode the frame
      my->drv->decode_frame(pos, inst->out_result->framebuffer, xsize, ysize);
    } else {
