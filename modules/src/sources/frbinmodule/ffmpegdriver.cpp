...
static int estimate_duration(AVFormatContext *av_fc, int stream_index,
                             double fps, double *duration, double *start_time);
...
static int decode(AVFormatContext *fc, int video_stream_index, AVFrame *frame,
  ...
  *got_frame = 0;
  ...
  if (video_stream->codec->codec_id == CODEC_ID_RAWVIDEO) {
    avpicture_fill((AVPicture *)frame, pkt->data, video_stream->codec->pix_fmt,
    ...
    *got_frame = 1;
    ...
    int len = avcodec_decode_video(video_stream->codec, frame, got_frame,
                                   pkt->data, pkt->size);
    if (len < 0) {
...
struct FFMpegDriverImpl {
  FFMpegDriverImpl()
      : av_fc(0), file_name(""), m_width(0), m_height(0), m_num_frames(0),
        video_stream_index(-1), m_frame(0), m_fps(25), m_scale_buf(0),
        m_scale_buf_size(0), m_start_time(0), m_current_timestamp(0) {
    av_register_all(); // register all decoders
  }

  ~FFMpegDriverImpl() { close(); }
  ...
  void open(const std::string &filename, VideoInfo &info) {
    ...
    int err = av_open_input_file(&av_fc, filename.c_str(), 0, 0, &params);
    ...
    double duration_s;
    int ret = estimate_duration(av_fc, video_stream_index, m_fps, &duration_s,
                                &m_start_time);
    if (ret < 0) {
      close();
    ...
    int num_frames = static_cast<int>(
        floor(av_q2d(video_stream->r_frame_rate) * duration_s));
    ...
    std::cout << "(width x height) = (" << info.width << "x" << info.height
              << ")\n";
  ...
  bool is_open() const { return av_fc != 0; }

  int num_frames() const { return m_num_frames; }

  void decode_frame(unsigned int frame_number, uint_32 *framebuffer, int width,
                    int height) {
    assert(av_fc != 0);
    ...
    if (fabs(wanted_timestamp - m_current_timestamp) >= 0.98 * frame_duration) {
      ...
      std::cout << "seeking to frame " << frame_number << ", time "
                << wanted_timestamp << "s\n";
#endif
      ...
      std::cout << "No need to seek: wanted_timestamp =" << wanted_timestamp
                ...
                << "s, diff = " << fabs(wanted_timestamp - m_current_timestamp)
                << "s)\n";
    }
#endif
    ...
    img_convert(&pict, dst_pix_fmt, (AVPicture *)m_frame,
                video_stream->codec->pix_fmt, video_stream->codec->width,
                video_stream->codec->height);

    if (cwidth != width || cheight != height) {
      ls_scale32(framebuffer, width, height,
                 reinterpret_cast<const uint_32 *>(pict.data[0]), cwidth,
                 cheight);
    } else {
      if (m_scale_buf) {
        delete[] m_scale_buf;
...
FFMpegDriver::FFMpegDriver() : m_impl(new FFMpegDriverImpl()) {}
...
void FFMpegDriver::close_file() { m_impl->close(); }

bool FFMpegDriver::is_open() const { return m_impl->is_open(); }

void FFMpegDriver::decode_frame(unsigned int frame_number, uint_32 *framebuffer,
...
static int estimate_duration(AVFormatContext *av_fc, int stream_index,
                             double fps, double *duration, double *start_time) {
  std::cout << "estimate_duration, stream_index = " << stream_index
            << ", fps = " << fps << "\n";
  ...
  double duration_0 = ((stream->duration == AV_NOPTS_VALUE)
                           ? frame_duration
  ...
  std::cout << "duration_0 = " << duration_0
            << " (stream->duration * time_base = " << stream->duration << " * "
            << time_base << " = " << (double)stream->duration * time_base
            << ")\n";
