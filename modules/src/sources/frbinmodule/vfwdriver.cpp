...
//----------------------------------------------------------------------
static int createDIB(HDC hdc, HBITMAP *p_hBitmap, uint_32 **p_data, int xsize,
                     int ysize) {
  ...
  *p_hBitmap = CreateDIBSection(hdc, (BITMAPINFO *)&bmih, DIB_RGB_COLORS,
                                (void **)p_data, NULL, 0);
...
public:
  ...
  void load(const std::string &filename, VideoInfo &info) {
    ...
    if (AVIFileGetStream(m_avifile, &m_pavi, streamtypeVIDEO, 0) != 0) {
      ...
      throw std::runtime_error("Failed To Open The AVI Stream: '" + filename +
                               "'");
    }
    ...
    if (it == s_get_stream_count.end()) {
      StreamGetList l;
      l.push_back(m_pgf);
      s_get_stream_count.insert(
          std::make_pair(filename.c_str(), std::make_pair(1, l)));
    } else {
      ++it->second.first;
  ...
  bool is_open() const { return m_hdc != 0; }
  ...
  void grab_frame(unsigned int frame, uint_32 *frb, int width, int height) {
    ...
    if (lpbi == 0) {
      ...
      if (m_hBitmap == 0 || m_bmp_width != width || m_bmp_height != height) {
        m_bmp_width = width;
        m_bmp_height = height;
      ...
      char *pdata =
          ((char *)lpbi) + lpbi->biSize + lpbi->biClrUsed * sizeof(RGBQUAD);

      // Convert Data To Requested Bitmap Format
      DrawDibDraw(m_hdd, m_hdc, 0, 0, width, height, lpbi, pdata, 0, 0, m_width,
                  m_height, 0);
...
VFWDriver::VFWDriver() : m_impl(new VFWDriverImpl()) {}
...
void VFWDriver::close_file() { m_impl->close(); }

bool VFWDriver::is_open() const { return m_impl->is_open(); }
