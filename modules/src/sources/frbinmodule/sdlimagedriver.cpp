...
public:
  SDLImageDriverImpl() : m_image(0), m_width(0), m_height(0) {}

  ~SDLImageDriverImpl() { close(); }
  ...
  void load(const std::string &filename, VideoInfo &info) {
    ...
    if (tmp_image == NULL) {
      ...
      os << "Couldn't load '" << filename << "' (" << SDL_GetError() << ")";
  ...
  void grab_frame(uint_32 *frb, unsigned int frame, int width, int height) {
    if (m_image == 0)
    ...
    ls_scale32(frb, width, height, (uint_32 *)m_image->pixels, m_width,
               m_height);
  }

  bool is_open() const { return m_image != 0; }
...
SDLImageDriver::SDLImageDriver() : m_impl(new SDLImageDriverImpl()) {}
...
bool SDLImageDriver::is_open() const { return m_impl->is_open(); }
