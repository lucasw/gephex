...
static BOOL GetFormattedError(LPTSTR dest, int size) {
  ...
  return 0 != FormatMessage(flags, NULL, dwLastError,
                            MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), dest,
                            size, NULL);
}
...
public:
  Callback(IMediaControl *media, const unsigned long &last_grab,
           const bool &in_grab_frame, unsigned long time_to_stop)
      : m_media(media), m_last_grab(last_grab), m_in_grab_frame(in_grab_frame),
...
public:
  DSHOWDriverImpl()
      : m_event(0), m_grabber_basefilter(0), m_graph(0), m_mediacontrol(0),
        m_null_renderer(0), m_samplegrabber(0), m_src(0), m_media_seek(0),
        m_width(0), m_height(0), m_tmp_data(0), m_last_grabtime(0), m_timer(0),
  ...
  void close() { destroyFilterGraph(); }

  void load(const std::string &filename, VideoInfo &info) {

    build_grabber_graph(&m_graph, &m_mediacontrol, &m_grabber_basefilter,
                        &m_samplegrabber, &m_null_renderer, 0);
    //&m_callback);
    ...
    MultiByteToWideChar(CP_ACP, 0, filename.c_str(), filename.length() + 1,
    ...
    connect_source_filter(m_graph, m_grabber_basefilter, m_samplegrabber,
                          m_null_renderer, m_src, m_width, m_height);

    info.width = m_width;
    info.height = m_height;

    hr = m_graph->QueryInterface(IID_IMediaSeeking, (void **)&m_media_seek);
    if (FAILED(hr)) {
      char temp[1024];
      AMGetErrorText(hr, temp, sizeof(temp));
    ...
    Callback *cb =
        new Callback(m_mediacontrol, m_last_grabtime, m_in_grab_frame, 500);
  ...
  void grab_frame(uint_32 *frb, int frame, int width, int height) {
    ...
    if (current != frame) {
      LONGLONG c = frame;
#if defined(SEGMENT_SEEK)
      hr = m_media_seek->SetPositions(
          &c,
          AM_SEEKING_AbsolutePositioning | AM_SEEKING_NoFlush ||
              AM_SEEKING_Segment,
          &c, AM_SEEKING_NoFlush || AM_SEEKING_Segment);
#else
      hr = m_media_seek->SetPositions(
          &c, AM_SEEKING_AbsolutePositioning | AM_SEEKING_NoFlush, 0,
    ...
    if (width == m_width && height == m_height) {
      ...
      ls_scale32(frb, width, height, m_tmp_data, m_width, m_height);
    }
  ...
  bool is_open() const { return m_graph != 0; }
...
DSHOWDriver::DSHOWDriver() : m_impl(new DSHOWDriverImpl()) {}
...
void DSHOWDriver::decode_frame(unsigned int frame_number, uint_32 *framebuffer,
                               int width, int height) {
  if (m_impl->is_open())
    m_impl->grab_frame(framebuffer, frame_number, width, height);
}

void DSHOWDriver::close_file() { m_impl->close(); }

bool DSHOWDriver::is_open() const { return m_impl->is_open(); }
