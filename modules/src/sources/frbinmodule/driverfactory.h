...
template <class DRV> class DriverConstructor : public IDriverConstructor {
public:
  DriverConstructor() {}
  ...
  VideoFileDriver *create() const { return new DRV(); }
};
...
public:
  ...
  DriverFactory(){};
  ~DriverFactory() {
    for (CtorList::iterator it = m_ctor_list.begin(); it != m_ctor_list.end();
         ++it) {
      delete *it;
    }
  }
   ...
   */
  std::list<IDriverConstructor *>
  get_drivers(const std::string &extension) const {
