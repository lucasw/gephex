...
struct AviFileDriverImpl {
  ...
  bool is_open() const { return videoFile != 0; }
...
AviFileDriver::AviFileDriver() : m_impl(new AviFileDriverImpl()) {}
...
void AviFileDriver::load_file(const std::string &file_name, VideoInfo &info) {
  ...
  try {
    ...
    m_impl->videoStream = m_impl->videoFile->GetStream(0, avm::IStream::Video);
    ...
    try {
      std::auto_ptr<avm::StreamInfo> sinfo(
          m_impl->videoStream->GetStreamInfo());
      ...
      info.num_frames = m_impl->nf = m_impl->videoStream->GetLength() - 1;
...
void AviFileDriver::close_file() { m_impl->close(); }

bool AviFileDriver::is_open() const { return m_impl->is_open(); }

void AviFileDriver::decode_frame(unsigned int frame_number,
                                 uint_32 *framebuffer, int width, int height) {
  ...
  if (!m_impl->videoStream->Eof()) {
    ...
    ls_scale32(framebuffer, width, height,
               reinterpret_cast<const uint_32 *>(m_impl->im->Data()), m_impl->w,
               m_impl->h);
  }
}
