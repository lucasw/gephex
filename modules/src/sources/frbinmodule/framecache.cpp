...
public:
  ...
  uint_32 *lookup(int pos) const {
    FrameVector::const_iterator it =
        std::lower_bound(m_frames.begin(), m_frames.end(),
                         std::make_pair(pos, (uint_32 *)0), compare_tag);
  ...
  void store(int pos, uint_32 *frb) {
    FrameVector::iterator it =
        std::upper_bound(m_frames.begin(), m_frames.end(),
                         std::make_pair(pos, (uint_32 *)0), compare_tag);
  ...
  ~Cache() {
    for (FrameVector::iterator it = m_frames.begin(); it != m_frames.end();
         ++it) {
      delete[] it->second;
    }
  }
...
struct CacheKey {
  ...
  bool operator==(const CacheKey &rhs) const {
    return file_name == rhs.file_name && width == rhs.width &&
           height == rhs.height;
  }
...
public:
  ...
  void store(const std::string &file_name, int frame_number, int width,
             int height, uint_32 *framebuffer) {
    ...
    if (m_size + size > m_max_size) {
      ...
      for (CacheList::const_iterator it = m_cache.begin(); it != m_cache.end();
           ++it) {
      ...
      if (min_it != m_cache.end()) {
        ...
        int num_frames =
            (int)(((double)((m_size + size) - m_max_size)) / size_per_frame +
                  0.5);
  ...
  uint_32 *lookup(const std::string &file_name, int frame_number, int width,
                  int height) {
    CacheList::iterator it = std::find(m_cache.begin(), m_cache.end(),
...
FrameCache::FrameCache(int size_in_mbytes) : m_impl(new Impl(size_in_mbytes)) {}
...
void FrameCache::flush() { m_impl->flush(); }
