...
template <typename INT> uint8_t uint8_t_sat(INT c) {
...
struct bgra_t {
  ...
  bgra_t(uint8_t red_ = 0, uint8_t green_ = 0, uint8_t blue_ = 0,
         uint8_t alpha_ = 0) {
    pixel[0] = blue_;
    pixel[1] = green_;
    pixel[2] = red_;
...
void cvt_yuv420p_to_bgra(uint8_t *dst, uint8_t *src, size_t width,
                         size_t height) {
  ...
  while (t != t_frame_end) {
    ...
    while (t != t_line_end) {
      ...
      uint8_t y_0 = y[0];
      t[0] = bgra_t(uint8_t_sat(y_0 + rv), uint8_t_sat(y_0 + gv + gu),
                    uint8_t_sat(y_0 + bu));

      uint8_t y_1 = y[1];
      t[1] = bgra_t(uint8_t_sat(y_1 + rv), uint8_t_sat(y_1 + gv + gu),
                    uint8_t_sat(y_1 + bu));

      uint8_t y_2 = y[width];
      t[width] = bgra_t(uint8_t_sat(y_2 + rv), uint8_t_sat(y_2 + gv + gu),
                        uint8_t_sat(y_2 + bu));

      uint8_t y_3 = y[width + 1];
      t[width + 1] = bgra_t(uint8_t_sat(y_3 + rv), uint8_t_sat(y_3 + gv + gu),
                            uint8_t_sat(y_3 + bu));
...
void cvt_rgb32_to_i420(uint8_t *dst, const uint32_t *src, size_t width,
                       size_t height, int *pitches, int *offsets) {
...
void cvt_rgb16_to_i420(uint8_t *dst, const uint16_t *src, size_t width,
                       size_t height, int *pitches, int *offsets) {
  // todo
