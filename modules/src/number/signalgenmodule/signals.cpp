...
double Sinus::inv(double val, double f, double amp, double delta_val) {
  double inv;
...
double Triangle::inv(double val, double f, double amp, double delta_val) {
  double T = 1.0 / f;
...
double Ramp::inv(double val, double f, double amp, double delta_val) {
  return val / (amp * f);
}
...
static void noise_fill_data(int size, double *values, int &pos, double f,
                            double amp) {
  int n = size;
...
Noise::Noise(int n) : m_values(new double[n]), m_size(n), m_pos(n) {
  m_values[n - 1] = 0;
}

Noise::~Noise() { delete[] m_values; }
...
SignalFactory::SignalFactory(const std::string &name) : m_name(name) {}
