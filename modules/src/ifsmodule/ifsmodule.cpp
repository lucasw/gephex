...
void update(void *instance) {
  ...
  if (my->frameBuffer == 0 || inst->out_out->xsize != my->xSize ||
      inst->out_out->ysize != my->ySize) {
    s_log(2, "change");
    delete[] my->frameBuffer;
  ...
  // clear background
  std::fill(my->frameBuffer, &(my->frameBuffer[my->xSize * my->ySize]), 0);
  ...
  if (inst->in_ifs->numfun != 0)
    for (int i = 0; i != numDots; ++i) {
      ...
      double xn = inst->in_ifs->ifs[0][fun] * x +
                  inst->in_ifs->ifs[1][fun] * y + inst->in_ifs->ifs[4][fun];

      double yn = inst->in_ifs->ifs[2][fun] * x +
                  inst->in_ifs->ifs[3][fun] * y + inst->in_ifs->ifs[5][fun];
      ...
      // clipping
      if ((x_pos > 0) && (x_pos < my->xSize) && (y_pos > 0) &&
          (y_pos < my->ySize)) {
        // increase lightness
  ...
  std::transform(my->frameBuffer, &(my->frameBuffer[my->xSize * my->ySize]),
