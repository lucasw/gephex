...
void update(void* instance)
{
  ...
  if (joy_id != my->old_id)
      ...
      if (my->jst == 0)
	{
      ...
      num_axes = joystick_get_num_axes(my->jst);
  ...
  joystick_poll(my->jst);

  num_axes = joystick_get_num_axes(my->jst);
  num_buttons = joystick_get_num_axes(my->jst);

  for (i = 0; i < num_axes; ++i)
    {
      ax[i] = joystick_get_axis(my->jst, i);	  
    }

  for (i = 0; i < num_buttons; ++i)
    {
      b[i] = joystick_get_button(my->jst, i);
    }
