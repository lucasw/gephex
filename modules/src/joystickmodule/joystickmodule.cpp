...
void update(void *instance) {
  ...
  if (my->jst_drv == 0 || strcmp(new_driver_name, driver_name) != 0) {
    if (my->jst_drv)
      delete my->jst_drv;
    ...
    try {
      ...
      char buffer[256];
      snprintf(buffer, sizeof(buffer), "Could not create joystick driver: '%s'",
               e.what());
      s_log(0, buffer);
    }
  }
  ...
  if (joy_id != my->old_id) {
    ...
    try {
      ...
      snprintf(buffer, sizeof(buffer), "Name: %s\t#Axes: %i\t#Buttons: %i",
               my->jst_drv->name(joy_id).c_str(), my->jst->num_axes(),
               my->jst->num_buttons());
      s_log(2, buffer);
    ...
    if (my->jst == 0) {
      snprintf(buffer, sizeof(buffer), "Couldn't open Joystick %i", joy_id);
      s_log(0, buffer);
      return;
    }
  ...
  try {
    ...
    char buffer[128];
    snprintf(buffer, sizeof(buffer), "Error at poll(): '%s'", e.what());
    s_log(0, buffer);
  }
}
