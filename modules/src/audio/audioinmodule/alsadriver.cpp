...
AlsaDriver::AlsaDriver() : m_impl(new Impl(0)) {}
...
void AlsaDriver::open(logT /*logger*/, device_id_t device, int sample_rate,
                      sample_format format, int channels) {
  if (m_impl->handle)
  ...
  err = snd_pcm_open(&capture_handle, buffer, SND_PCM_STREAM_CAPTURE,
                     SND_PCM_NONBLOCK);
  ...
  if (err < 0) {
    std::ostringstream os;
    os << "cannot set channel count (" << snd_strerror(err) << ")";
    snd_pcm_hw_params_free(hw_params);
    snd_pcm_close(m_impl->handle);
    throw std::runtime_error(os.str().c_str());
  }
...
int AlsaDriver::read(unsigned char *data, int num_samples) {
  ...
  } else if (err == -EAGAIN) // nothing to read
  {
    ...
    std::ostringstream os;
    os << "read from audio interface failed (" << snd_strerror(err) << ")";
    throw std::runtime_error(os.str().c_str());
  }
...
bool AlsaDriver::is_open() const { return m_impl->handle != 0; }
//---------------------------------------------------------------------------
