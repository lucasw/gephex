...
void MutexPOSIX::acquire() { pthread_mutex_lock((pthread_mutex_t *)m_mutex); }

void MutexPOSIX::release() { pthread_mutex_unlock((pthread_mutex_t *)m_mutex); }
...
struct print_devname {
  ...
  void operator()(AudioDeviceID id) const {
    ...
    OSStatus err = AudioDeviceGetProperty(
        id, 0, true, kAudioDevicePropertyDeviceName, &maxlen, buf);
...
AudioDeviceID GetDeviceID(logT logger, int dev) {
  ...
  err = AudioHardwareGetPropertyInfo(kAudioHardwarePropertyDevices, &propsize,
                                     NULL);
  ...
  err = AudioHardwareGetProperty(kAudioHardwarePropertyDevices, &propsize,
                                 &devids[0]);
...
void print_format::operator()(const AudioStreamBasicDescription &format) const {
  ...
  sprintf(buffer,
          ...
          "ChannelsPerFrame = %i\n"
          "BitsPerChannel   = %i\n",
          m_title, format.mSampleRate, formatID, format.mFormatFlags,
...
struct print_configuration {
  ...
  void operator()(const AudioBufferList &buffer) const {
    ...
    sprintf(buf, " - AudioBufferList, numBuffers     = %i\n",
            buffer.mNumberBuffers);

    m_logger(2, buf);

    for (int i = 0; i < buffer.mNumberBuffers; ++i) {
      sprintf(buf, "   + buffer #channels = %i, data byte size = %i\n",
...
void print_info(AudioDeviceID devid, logT logger, bool isInput) {
  ...
  err = AudioDeviceGetProperty(
      devid, 0, isInput, kAudioDevicePropertyStreamFormat, &propsize, &format);
  ...
  std::vector<AudioStreamBasicDescription> formats =
      get_prop_list<AudioStreamBasicDescription>(
          devid, kAudioDevicePropertyStreamFormats, isInput);

  std::for_each(formats.begin(), formats.end(),
                print_format(logger, "Supported device stream format\n"));

  std::vector<AudioStreamID> streams =
      get_prop_list<AudioStreamID>(devid, kAudioDevicePropertyStreams, isInput);

  for (std::vector<AudioStreamID>::const_iterator it = streams.begin();
       it != streams.end(); ++it) {
    AudioStreamBasicDescription format;
    propsize = sizeof(format);
    err = AudioStreamGetProperty(*it, 0, kAudioStreamPropertyPhysicalFormat,
                                 &propsize, &format);
    ...
    std::vector<AudioStreamBasicDescription> stream_formats =
        get_stream_prop_list<AudioStreamBasicDescription>(
            *it, kAudioStreamPropertyPhysicalFormats);

    std::for_each(stream_formats.begin(), stream_formats.end(),
                  print_format(logger, "Supported physical stream format\n"));
  }

  std::vector<AudioBufferList> configuration = get_prop_list<AudioBufferList>(
      devid, kAudioDevicePropertyStreamConfiguration, isInput);
