...
public:
  ConversionHelper(AudioConverterRef converter, const unsigned char *input_data,
                   int num_input_data_bytes, int sourceChannels,
                   int sourceBytesPerPacket, int destChannels,
                   int destBytesPerPacket)
      : m_converter(converter), m_data(input_data),
        m_num_data_bytes(num_input_data_bytes),
        m_sourceChannels(sourceChannels),
        m_sourceBytesPerPacket(sourceBytesPerPacket),
        m_destChannels(destChannels), m_destBytesPerPacket(destBytesPerPacket),
        m_pos(0) {}
  ...
  OSStatus convert(unsigned char *output_data, UInt32 *output_size_bytes) {
    ...
    OSStatus err = AudioConverterFillComplexBuffer(
        m_converter, inputProc, this, &ioOutputDataPacketSize, &bufferList, 0);
  ...
  static OSStatus
  inputProc(AudioConverterRef inAudioConverter, UInt32 *ioNumberDataPackets,
...

  {
    ...
    int numBytes =
        my_min<int>(*ioNumberDataPackets * helper->m_sourceBytesPerPacket,
                    helper->m_num_data_bytes - helper->m_pos);

    ioData->mBuffers[0].mNumberChannels = helper->m_sourceChannels;

    ioData->mBuffers[0].mData =
        (numBytes > 0) ? &helper->m_data[helper->m_pos] : 0;
...
OSStatus StereoFloatToMono16LEPCM::convert(const unsigned char *input_data,
  ...
  const int inputDataSizeBytes =
      my_min(num_input_bytes, size_output_buffer * 8 / 2);
...
OSStatus Mono16LEPCMToStereoFloat::convert(const unsigned char *input_data,
                                           ...
                                           int &size_output_buffer) {
  const int inputDataSizeBytes =
      my_min(num_input_bytes, size_output_buffer * 2 / 8);
...
CoreAudioConverterBase::CoreAudioConverterBase(
  ...
  OSStatus err =
      AudioConverterNew(&m_sourceFormat, &m_destFormat, &m_converter);
...
OSStatus SimpleConverter::convert(const unsigned char *input_data,
  ...
  OSStatus err =
      AudioConverterConvertBuffer(m_converter, inputDataSizeBytes, input_data,
                                  &outputDataSizeBytes, output_data);
...
OSStatus ComplexConverter::convert(const unsigned char *input_data,
  ...
  ConversionHelper helper(
      m_converter, input_data, num_input_bytes,
      m_sourceFormat.mChannelsPerFrame, m_sourceFormat.mBytesPerPacket,
      m_destFormat.mChannelsPerFrame, m_destFormat.mBytesPerPacket);
