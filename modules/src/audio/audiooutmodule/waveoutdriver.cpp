...
WaveOutDriver::WaveOutDriver() : m_impl(new Impl()) {}
...
void WaveOutDriver::open(device_id_t device, int sample_rate,
  ...
  m_impl->buf =
      outb_create(m_impl->handle, num_samples * bytes_per_sample, num_periods);
...
int WaveOutDriver::write(const unsigned char *data, int num_samples) {
  ...
  int bytes_written =
      outb_write(m_impl->buf, data, num_samples * m_impl->bytes_per_sample,
...
bool WaveOutDriver::is_open() const { return m_impl->handle != 0; }
