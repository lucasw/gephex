...
struct CoreAudioOutDriver::Impl {
  enum IOProcState { kOff, kStarting, kRunning, kStopRequested };

  Impl()
      : m_outputProcState(kOff), m_ID(kAudioDeviceUnknown), m_min_buffer(0) {}

  void new_data(const unsigned char *data, UInt32 numBytes) {
    MutexHolder mh(m_mutex);

    // Buffer at most 2 s of audio samples
    const size_t MAX_BUFFER = static_cast<size_t>(
        2 * m_input_format.mBytesPerFrame * m_format.mSampleRate);

    assert(numBytes < MAX_BUFFER);

    if (m_buffer.size() + numBytes > MAX_BUFFER) {
      const size_t toRemove =
          my_min(m_buffer.size(), m_buffer.size() + numBytes - MAX_BUFFER);

      m_buffer.erase(m_buffer.begin(), m_buffer.begin() + toRemove);
    }

    m_buffer.insert(m_buffer.end(), data, data + numBytes);

    assert(m_buffer.size() <= MAX_BUFFER);

    if (m_outputProcState == kOff && m_buffer.size() >= m_min_buffer) {
      start_audio();
    }
  }
  ...
  int read(unsigned char *data, int num_samples) {
    ...
    const UInt32 inputDataSizeBytes =
        my_min(num_samples * m_input_format.mBytesPerFrame, m_buffer.size());

    std::vector<unsigned char> input_data(
        m_buffer.begin(), m_buffer.begin() + inputDataSizeBytes);

    m_buffer.erase(m_buffer.begin(), m_buffer.begin() + inputDataSizeBytes);

    int outputDataSizeBytes = num_samples * m_format.mBytesPerFrame;

    OSStatus err;

    err = m_converter->convert(&input_data[0], input_data.size(), data,
                               outputDataSizeBytes);
    ...
    if (err != kAudioHardwareNoError) {
      ...
      sprintf(buf,
              ...
              num_samples, inputDataSizeBytes, m_buffer.size(),
              outputDataSizeBytes, err, err);
...
CoreAudioOutDriver::CoreAudioOutDriver() : m_impl(new Impl()) {}
...
namespace {
void logger(int, const char *s) { printf("%s\n", s); }
} // namespace

void CoreAudioOutDriver::open(device_id_t device, int sample_rate,
                              sample_format form, int channels, int num_samples,
                              int num_periods) {
  if (this->is_open())
  ...
  UInt32 frames =
      my_min<int>(my_max<int>(num_samples, (int)bufferRange.mMinimum),
  ...
  err = AudioDeviceSetProperty(devid, NULL, 0, isInput,
                               kAudioDevicePropertyNominalSampleRate, propsize,
                               &sampleRate);
  ...
  err = AudioDeviceGetProperty(devid, 0, isInput,
                               kAudioDevicePropertyStreamFormat, &propsize,
                               &m_impl->m_format);
  ...
  m_impl->m_input_format.mBytesPerFrame =
      m_impl->m_input_format.mBytesPerPacket;
...
#define USE_COMPLEX_AUDIO_CONVERTER
  ...
  m_impl->m_converter =
      new SimpleConverter(m_impl->m_input_format, m_impl->m_format);
#elif defined(USE_COMPLEX_AUDIO_CONVERTER)
  m_impl->m_converter =
      new ComplexConverter(m_impl->m_input_format, m_impl->m_format);
#else
