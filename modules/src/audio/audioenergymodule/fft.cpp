...
void fftInplace(std::vector<std::complex<_REAL>> &vals, int dir) {
  ...
  for (int m = 2; m <= n; m <<= 1) {
    std::complex<_REAL> wm =
        std::exp(dir * 2 * M_PI * std::complex<_REAL>(0, 1) / _REAL(m));
...
void fftRealRecursive(const std::vector<_REAL> &vals1,
                      ...
                      std::vector<_REAL> &rvals1, std::vector<_REAL> &rvals2,
                      int dir) {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    rvals2[i] = (std::complex<_REAL>(0, -1) * (rcvals[i] - conj(rcvals[N - i])))
                    .real() /
                2.0;
  }
}
...
template <typename _REAL>
void fftRealInplace(std::vector<_REAL> &vals1, std::vector<_REAL> &vals2,
                    int dir) {
  assert(dir == 1 || dir == -1);
  assert(vals1.size() == vals2.size());
  ...
  for (int i = 0; i < n; ++i) {
    ...
    vals2[i] =
        (std::complex<_REAL>(0, -1) * (cvals[i] - conj(cvals[n - i]))).real() /
        2;
  }
}
