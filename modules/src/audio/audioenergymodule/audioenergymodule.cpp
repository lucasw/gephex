...
public:
  SampleBuffer(int block_size)
      : m_size(0), m_num_blocks(0), m_block_size(block_size), m_blocks(0) {
  ...
  void put(int num_samples, const double *samples) {
    ...
    if (m_num_blocks + needed_blocks > m_size) {
      ...
      if (m_num_blocks > 0) {
        memcpy(new_blocks, m_blocks, m_num_blocks * sizeof(double));
        delete[] m_blocks;
      }
    ...
    if (rest != 0) {
      double sum = 0;
      for (int j = m_num_blocks * m_block_size; j < num_samples; ++j) {
        sum += samples[j];
      }
