...
void update(void *instance) {
  ...
  for (uint_32 *pixel = fb; pixel != fb + (attribs.xsize * attribs.ysize);
       ++pixel) {
    *pixel = 0;
  }
  ...
  if (!trim_bool(inst->in_start->number)) {
    ...
    if (ball_x < 12 && (inst->out_speed_x->number < 0) &&
        (ball_y >= pos1 && ball_y <= (pos1 + 10))) {
      inst->out_speed_x->number = -inst->in_speed_x->number;
      inst->out_status->number = 1;
    }

    if (ball_x > 148 && (inst->out_speed_x->number > 0) &&
        (ball_y >= pos2 && ball_y <= (pos2 + 10))) {
      inst->out_speed_x->number = -inst->in_speed_x->number;
      inst->out_status->number = 2;
    }

    inst->out_ball_x->number =
        trim_double(inst->out_ball_x->number + inst->out_speed_x->number, 0, 1);

    inst->out_ball_y->number =
        trim_double(inst->out_ball_y->number + inst->out_speed_y->number, 0, 1);
  }
