...
static int write_data(int fd, const unsigned char *buf, int buf_len);

//---------------------------------------------------------------------------

OSSOutDriver::OSSOutDriver() : m_impl(new Impl()) {}

OSSOutDriver::~OSSOutDriver() {
  if (this->is_open())
    this->close();
}

void OSSOutDriver::open(device_id_t device, int sample_rate,
                        sample_format format, int channels,
                        int samples_per_period, int num_periods) {
  if (this->is_open())
...
int OSSOutDriver::write(const unsigned char *data, int num_samples) {
  if (!this->is_open())
    throw std::logic_error("Device not open");

  int len =
      write_data(m_impl->fd, data, num_samples * m_impl->bytes_per_sample);
...
bool OSSOutDriver::is_open() const { return m_impl->fd != -1; }
...
static int open_device(const char *device_name, int *fd) {
  ...
  *fd = open(device_name, O_WRONLY, 0);
  if (*fd < 0)
    return 0;
