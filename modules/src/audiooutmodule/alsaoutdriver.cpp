...
AlsaOutDriver::AlsaOutDriver() : m_impl(new Impl()) {}
...
void AlsaOutDriver::open(device_id_t device, int sample_rate,
                         sample_format format, int channels, int num_samples,
                         int num_periods) {
  if (this->is_open())
  ...
  err =
      snd_pcm_open(&handle, buffer, SND_PCM_STREAM_PLAYBACK, SND_PCM_NONBLOCK);
  if (err < 0) {
    std::ostringstream os;
    os << "cannot open audio device " << buffer << "(" << snd_strerror(err)
       << ")";
  ...
  if (err < 0) {
    std::ostringstream os;
    os << "cannot allocate hardware parameter structure (" << snd_strerror(err)
       << ")";
  ...
  err = snd_pcm_hw_params_set_rate(handle, hw_params, sample_rate, 0);
  if (err < 0) {
    std::ostringstream os;
  ...
  err = snd_pcm_hw_params_set_buffer_size(
      handle, hw_params, (num_samples * num_periods) / bytes_per_sample);
  if (err < 0) {
    std::ostringstream os;
  ...
  if (err < 0) {
    std::ostringstream os;
    os << "cannot prepare audio interface for use (" << snd_strerror(err)
       << ")";
    snd_pcm_close(m_impl->handle);
    throw std::runtime_error(os.str().c_str());
  }
...
int AlsaOutDriver::write(const unsigned char *data, int num_samples) {
  ...
  memcpy(m_impl->buffer + m_impl->buf_pos, data,
  ...
  memmove(m_impl->buffer, m_impl->buffer + err,
          (m_impl->buf_pos - err) * m_impl->bytes_per_sample);
  m_impl->buf_pos -= err;

  return err;
}

bool AlsaOutDriver::is_open() const { return m_impl->handle != 0; }
