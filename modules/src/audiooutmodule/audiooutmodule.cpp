...
void update(void *instance) {
  ...
  if (inst->in_audio->sample_rate != SAMPLE_RATE ||
    ...
    snprintf(buffer, sizeof(buffer), "sample_rate = %i; channels = %i",
             inst->in_audio->sample_rate, inst->in_audio->channels);
    s_log(0, buffer);
...
#error No audio driver !
  ...
  if (device != my->device_number || latency != my->latency ||
    ...
    const int num_periods =
        (int)max(1, (int)(0.5 + (((double)latency / 1000.) * SAMPLE_RATE) /
                                    NUM_SAMPLES));

    try {
      my->drv->open(device, SAMPLE_RATE, SAMPLE_FORMAT, CHANNELS, NUM_SAMPLES,
                    num_periods);
