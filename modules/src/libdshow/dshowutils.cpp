...
static HRESULT ConnectFilters(IGraphBuilder *graph, IBaseFilter *lhs,
                              IBaseFilter *rhs) {
  HRESULT hr = S_OK;
  IPin *out = 0;
  IPin *in = 0;
...
void build_grabber_graph(IGraphBuilder **graph, IMediaControl **mediacontrol,
  ...
  hr = ::CoCreateInstance(CLSID_SampleGrabber, 0, CLSCTX_INPROC_SERVER,
                          IID_IBaseFilter, (void **)grabber_basefilter);
  ...
  (*grabber_basefilter)
      ->QueryInterface(IID_ISampleGrabber, (void **)samplegrabber);
  if (*samplegrabber == 0) {
