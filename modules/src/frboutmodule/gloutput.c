                 ...
                 char* error_text, int text_len)
{
  ...
  if (sh == 0) 
    {
      snprintf(error_text, text_len, "GL_new_instance: Could not allocate memory");
      return 0;
    }

  sh->win_xsize = width;
  sh->win_ysize = height;
  // resize texture lazy during blit
  sh->tex_xsize = 1; 
  sh->tex_ysize = 1;
  
  if(!initOutput("Ge-Phex Output Window", sh->win_xsize, sh->win_ysize, 32))
    {
      free(sh);
      snprintf(error_text, text_len, 
	       "GL: Could not initialize the Output window!");
      return 0; // error
    }	
