...
static struct DriverInstance *GL_new_instance(const char *server_name, int xpos,
...
static int GL_blit(struct DriverInstance *sh, const unsigned char *fb,
                   int width, int height, struct blit_params *params,
                   char *error_text, int buflen);
...
static struct DriverInstance *GL_new_instance(const char *server_name, int xpos,
  ...
  try {
    ...
    if (!initOutput("Ge-Phex OpenGL Output Window", server_name, sh->win_xsize,
                    sh->win_ysize, 32)) {
...
static int GL_blit(struct DriverInstance *sh, const unsigned char *fb,
                   int img_xsize, int img_ysize, struct blit_params *params,
                   char *error_text, int buflen) {
  double mirror_x;
  if (params->mirrorx)
    mirror_x = -1;
  ...
  while (new_tex_xsize < img_xsize)
    new_tex_xsize *= 2;
  while (new_tex_ysize < img_ysize)
    new_tex_ysize *= 2;

  if ((new_tex_xsize != sh->tex_xsize) || (new_tex_ysize != sh->tex_ysize)) {
    // alloc the texture
    char *buf = (char *)malloc(new_tex_xsize * new_tex_ysize * 4);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, new_tex_xsize, new_tex_ysize, 0,
  ...
  // blit with texture
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, img_xsize, img_ysize, GL_BGRA,
                  GL_UNSIGNED_BYTE, fb);
