...
enum scroll_mode_t { VERTICAL = 0, HORIZONTAL, SINE, STATIC };
...
static std::string get_real_filename(const std::string &fname) {
  ...
  int ret = GetEnvironmentVariable(GEPHEX_MEDIA_PATH, media_path_buffer,
...
int fontChanged(MyInstance *my, const char *font, int bold, int italic,
                int underlined, int psize = DEFAULT_POINT_SIZE) {
  if (my->font) {
    TTF_CloseFont(my->font);
    my->font = 0;
  }
...
void update(void *instance) {
  ...
  if (width != my->lastXSize || height != my->lastYSize) {
    ...
    my->scratch = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 32, rmask,
                                       gmask, bmask, amask);
  }

  bool renderFont = false;
  if (my->font == 0 || *my->lastFont != inst->in_font->text ||
      bold != my->bold || italic != my->italic ||
      underlined != my->underlined || point_size != my->lastPointSize) {
    if (fontChanged(my, inst->in_font->text, bold, italic, underlined,
                    point_size) == 0) {
      return;
    }
  ...
  if (!my->text) {
    s_log(0,
          "Call to TTF_RenderText_Solid returned NULL-surface, returning...");
    return;
  }
