...
typedef void (*midi_noteoffT)(uint8_t channel, uint8_t key, uint8_t velocity,
                              void *user_data);

typedef void (*midi_noteonT)(uint8_t channel, uint8_t key, uint8_t velocity,
                             void *user_data);

typedef void (*midi_aftertouchT)(uint8_t channel, uint8_t key, uint8_t pressure,
                                 void *user_data);

typedef void (*midi_controlchangeT)(uint8_t channel, uint8_t control,
                                    uint8_t value, void *user_data);

typedef void (*midi_programchangeT)(uint8_t channel, uint8_t program,
                                    void *user_data);

typedef void (*midi_channelaftertouchT)(uint8_t channel, uint8_t pressure,
                                        void *user_data);

typedef void (*midi_pitchbendT)(uint8_t channel, int value, void *self);
 ...
 */
struct MidiParser *midi_create_parser(
    midi_noteoffT noteoff, midi_noteonT noteon, midi_aftertouchT aftertouch,
    midi_controlchangeT controlchange, midi_programchangeT programchange,
    midi_channelaftertouchT channelaftert, midi_pitchbendT pitchbend,
    midi_systemexclusiveT systemexclusive, void *user_data);
 ...
 */
void midi_set_noteoff_handler(struct MidiParser *self, midi_noteoffT handler);
