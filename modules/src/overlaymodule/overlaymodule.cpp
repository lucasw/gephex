...
struct bgra_pixel_t {
  ...
  uint8_t gray() { return (red() + 2 * green() + blue()) / 4; }

  void alpha_mix(uint8_t a, bgra_pixel_t &color1, bgra_pixel_t &color2) {
    uint8_t ia = 255 - a;
...
static __inline int
framebuffer_compare_attributes(FrameBufferAttributes *attributes1,
                               FrameBufferAttributes *attributes2) {
  return (attributes1->xsize == attributes2->xsize &&
          attributes1->ysize == attributes2->ysize);
}
...
void update(void *instance) {
  ...
  if (!framebuffer_compare_attributes(
          &in_1_attr,
          &in_control_attr)) { // we need to convert th control image
  ...
  if (!(framebuffer_compare_attributes(
          &in_1_attr, &in_2_attr))) { // scale the second input frame
  ...
  bgra_pixel_t *in_control_ptr =
      reinterpret_cast<bgra_pixel_t *>(in_control->data);
  ...
  if (alpha_mode)
    while (in_1_ptr != in_1_ptr_end) {
      *out_r_ptr =
          (in_control_ptr->gray() > tolerance) ? (*in_1_ptr) : (*in_2_ptr);
