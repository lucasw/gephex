...
WaveMidiInDriver::WaveMidiInDriver() : m_impl(new Impl()) {}
...
bool WaveMidiInDriver::is_open() const { return m_impl->open; }
...
static void midi_init(int device_num) {
  ...
  MMRESULT res = midiInOpen(&s_midi_in, device_num, (DWORD)midiCallBack,
  ...
  if (MMSYSERR_NOERROR != res) {
    ...
    snprintf(buffer, sizeof(buffer), "Failed to open MIDI input device: %s",
             errorText);
  ...
  if (MMSYSERR_NOERROR != res) {
    ...
    TCHAR errorText[256];
    midiInGetErrorText(res, errorText, 255);
    snprintf(buffer, sizeof(buffer), "Failed to prepare MIDI header: %s",
             errorText);
    throw std::runtime_error(buffer);
  }
  ...
  if (MMSYSERR_NOERROR != res) {
    ...
    TCHAR errorText[256];
    midiInGetErrorText(res, errorText, 255);
    snprintf(buffer, sizeof(buffer), "Failed to add MIDI buffer: %s",
             errorText);
    throw std::runtime_error(buffer);
  }
  ...
  if (MMSYSERR_NOERROR != res) {
    ...
    TCHAR errorText[256];
    midiInGetErrorText(res, errorText, 255);
    snprintf(buffer, sizeof(buffer), "Failed to start MIDI input device: %s",
             errorText);
...
static void CALLBACK midiCallBack(HMIDIIN hMidiIn, UINT wMsg, DWORD dwInstance,
  ...
  case MIM_DATA: {
    ...
    if (len < 0) {
      char buffer[64];
      snprintf(buffer, sizeof(buffer), "midi: unknown: %i %i %i", midiStat,
