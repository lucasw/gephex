...
static void sReceiveMidi(const MIDIPacketList *pktlist, void *refCon,
                         void * /*connRefCon*/);

//-------------------------------------------------------------------------

struct MacOSXMidiInDriver::Impl {
  Impl() : m_client(0), m_inPort(0) { pthread_mutex_init(&m_mutex, 0); }
  ...
  void open(int device) {
    ...
    CFStringRef name = CFStringCreateWithBytes(
        0, reinterpret_cast<const UInt8 *>(device_name.c_str()),
        device_name.length(), kCFStringEncodingUTF8, false);
    ...
    CFStringRef in_name = CFStringCreateWithBytes(
        0, reinterpret_cast<const UInt8 *>(input_name.c_str()),
        input_name.length(), kCFStringEncodingUTF8, false);

    //    err = MIDIInputPortCreate(m_client, CFSTR("Input port"),
    //		      sReceiveMidi, (void*)this, &m_inPort);
    err = MIDIDestinationCreate(m_client, in_name, sReceiveMidi, (void *)this,
                                &m_inPort);
...
MacOSXMidiInDriver::MacOSXMidiInDriver() : m_impl(new Impl()) {}
...
bool MacOSXMidiInDriver::is_open() const { return m_impl->is_open(); }
...
int MacOSXMidiInDriver::Impl::read_buffer(unsigned char *data, int data_size) {
  ...
  const int bytes_copied =
      std::min(data_size, static_cast<int>(m_buffer.size()));

  std::copy(m_buffer.begin(), m_buffer.begin() + bytes_copied, data);
...
void MacOSXMidiInDriver::Impl::put_bytes(
    const std::vector<unsigned char> &data) {
  pthread_mutex_lock(&m_mutex);

  const size_t to_copy = std::min(data.size(), MIDI_BUF_SIZE - m_buffer.size());
...
static void sReceiveMidi(const MIDIPacketList *pktlist, void *refCon,
                         void * /*connRefCon*/) {
  MacOSXMidiInDriver::Impl *m =
      reinterpret_cast<MacOSXMidiInDriver::Impl *>(refCon);
  ...
  for (unsigned int j = 0; j < pktlist->numPackets; ++j) {
    tmp.insert(tmp.end(), packet->data, packet->data + packet->length);
