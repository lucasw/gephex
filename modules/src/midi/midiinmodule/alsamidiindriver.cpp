...
AlsaMidiInDriver::AlsaMidiInDriver() : m_impl(new Impl()) {}
...
void AlsaMidiInDriver::open(device_id_t device) {
  ...
  if (err) {
    ...
    error_msg << "snd_rawmidi_open '" << device_name << "' failed: " << err;
    m_impl->handle_in = 0;
...
bool AlsaMidiInDriver::is_open() const { return m_impl->handle_in != 0; }
