...
void noteoff_callback(uint8_t channel, uint8_t key, uint8_t velocity,
                      void *user_data) {
  MyInstance *my = static_cast<MyInstance *>(user_data);

  if (my->channel == channel && my->note_nr_min <= key &&
      my->note_nr_max >= key) {
    my->pressed = false;
    my->event = true;
    my->rvelo = velocity;
...
void noteon_callback(uint8_t channel, uint8_t key, uint8_t velocity,
                     void *user_data) {
  MyInstance *my = static_cast<MyInstance *>(user_data);
...
MyInstance *construct() {
  ...
  my->parser = midi_create_parser(noteoff_callback, noteon_callback, 0, 0, 0, 0,
                                  0, 0, my);
  return my;
}
