...
struct Mpeg3DriverImpl {
  Mpeg3DriverImpl()
      : file(0), file_name(""), w(0), h(0), l(0), rows(0), num_rows(0),
        stream(0) {}
...
Mpeg3Driver::Mpeg3Driver() : m_impl(new Mpeg3DriverImpl()) {}
...
void Mpeg3Driver::load_file(const std::string &file_name, VideoInfo &info) {
  ...
  if (mpeg3_check_sig(const_cast<char *>(m_impl->file_name.c_str())) == 1) {
    ...
    } catch (std::runtime_error &e) { // log and clean up
      // s_log(2,e.what());
      mpeg3_close(m_impl->file);
      m_impl->file = 0;
    }
...
bool Mpeg3Driver::is_open() const { return m_impl->file != 0; }
...
void Mpeg3Driver::decode_frame(unsigned int frame_number, uint_32 *framebuffer,
  ...
  // decode one frame
  int errorCode = mpeg3_read_frame(m_impl->file, m_impl->rows, 0, 0, m_impl->w,
                                   m_impl->h,     // input window
                                   width, height, // output window
                                   MPEG3_BGRA8888, m_impl->stream);
  if (errorCode == -1)
