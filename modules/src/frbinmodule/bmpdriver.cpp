...
typedef struct bitmapFileHeader_ {
  ...
  int_16 bfReserved1;
  int_16 bfReserved2;
  uint_32 bfOffBits; // Offset in *bytes* (not bits) from fileheader to bmp-bits
                     // (includes palette)
} BitmapFileHeader;
 ...
 */
static int convert32(uint_8 *newData, const uint_8 *data, int_32 width,
                     int_32 height, int_16 bpp, RGBQuadPtr pal) {
...
class BMPDriverImpl {
public:
  BMPDriverImpl() : m_data(0), m_width(0), m_height(0) {}
  ...
  void load(const std::string &filename, VideoInfo &info) {
    ...
    if (bitmap.fileHeader.bfType != 0x4D42) {
      char buffer[128];
      snprintf(buffer, sizeof(buffer), "Wrong bfType in fileheader: '%X'",
    ...
    // if width is not divisible by 4 we have to do something...
    int padding =
        (bitmap.infoHeader.biWidth * (bitmap.infoHeader.biBitCount >> 3)) &
        0x03;
    padding = (padding == 0) ? 0 : 4 - padding;
    ...
    if (bitmap.infoHeader.biBitCount == 8) {
      // read Palette Info
      int colors_used =
          (bitmap.fileHeader.bfOffBits - 14 - bitmap.infoHeader.biSize) /
          sizeof(RGBQuad);
    ...
    if (!(bitmap.infoHeader.biBitCount == 8 ||
          ...
          bitmap.infoHeader.biBitCount == 32)) {
      fclose(file);
    ...
    int result = convert32((uint_8 *)m_data, (uint_8 *)bitmap.data, m_width,
                           m_height, bitmap.infoHeader.biBitCount, palette);

    free(bitmap.data);

    if (result == 0) {
      char buffer[128];
      snprintf(buffer, sizeof(buffer), "Unsupported BitsPerPixel value: '%d'",
  ...
  bool is_open() const { return m_data != 0; }
...
BMPDriver::BMPDriver() : m_impl(new BMPDriverImpl()) {}
...
bool BMPDriver::is_open() const { return m_impl->is_open(); }
