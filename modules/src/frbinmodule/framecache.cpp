...
public:
	...
	{
		FrameVector::const_iterator it = std::lower_bound(m_frames.begin(),
			m_frames.end(),
	...
	{
		FrameVector::iterator it = std::upper_bound(m_frames.begin(),
			m_frames.end(),
			std::make_pair(pos, (uint_32*) 0),
			compare_tag);
		...
		int width, int height, uint_32* framebuffer)
	{
		...
		if (m_size + size > m_max_size)
		{
			...
			{			
				...
				int num_frames 
					= (int) (((double)(m_max_size - (m_size + size))) / size_per_frame +0.5);
