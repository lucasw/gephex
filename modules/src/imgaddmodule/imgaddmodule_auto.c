...
//#include <string.h>
#include "dllutils.h"
#include "imgaddmodule.xpm"

const char* getSpec(void) {
 return "mod_spec { name=[mod_imgaddmodule] number_of_inputs=[3] number_of_outputs=[1] deterministic=[true] }";
}
...
{
  static const char* INFO = "info { name=[Bild-Addierer] group=[Mixer] inputs=[3 Amount Bild1 Bild2 ] outputs=[1 Bild1+Bild2 ] type=xpm } ";
  char* tmpBuf;
  int reqLen = 1 + strlen(INFO) + getSizeOfXPM(imgaddmodule_xpm);
  if (buf != 0 && reqLen <= bufLen)
    {
      char* offset;
      int i;
      int lines = getNumberOfStringsXPM(imgaddmodule_xpm);
      tmpBuf = malloc(reqLen);
      memcpy(tmpBuf,INFO,strlen(INFO)+1);
      offset = tmpBuf + strlen(INFO) + 1;
      for (i = 0; i < lines; ++i)
	{
	  char* source = imgaddmodule_xpm[i];
