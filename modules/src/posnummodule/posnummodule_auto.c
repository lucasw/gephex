...
const char* getSpec(void) {
 return "mod_spec { name=[mod_pos2num] number_of_inputs=[1] number_of_outputs=[2] deterministic=[true] }";
}
...
const char* getOutputSpec(int index) {
 switch(index) {
   case 0:
    return "output_spec { type=typ_NumberType id=xresult } ";
  break;
  case 1:
    return "output_spec { type=typ_NumberType id=yresult } ";
  break;
 }
 return 0;
}
...
int setOutput(void* instance,int index, void* typePointer)
{
 ...
 switch(index) {
  case 0:
   inst->out_xresult = (NumberType* ) typePointer;
  break;
  case 1:
   inst->out_yresult = (NumberType* ) typePointer;
  break;
 } //switch(index) 
 return 0;
}
  ...
  if (buf != 0 && reqLen <= bufLen)
    {
      ...
      for (i = 0; i < lines; ++i)
        {
          char* source = posnummodule_xpm[i];
