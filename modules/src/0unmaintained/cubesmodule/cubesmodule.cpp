...
public:
  ...
  void getNormal() {
    ...
    tmp.Set(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z,
            a.x * b.y - a.y * b.x);
...
// ---------------------------------------------------------------------------
inline void DrawScanline(uint_32 *buffer, tmpVector p1, tmpVector p2,
                         Color color) {
  int x = 0;
  float z = p1.z;
...
//
//=============================================================================
template <typename type = double> class point_3d {
public:
  type x, y, z;
};

template <typename type = double> class TVertex {
private:
  type z;
...
#define f(i, j, k)                                                             \
  g_pPotGrid[(i) + (j) * (mbnx + 1) + (k) * (mbnx_x_mbny + mbnx + mbny + 1)]
  ...
  for (k = 0; k <= mbnz; k++) {
    ...
    for (j = 0; j <= mbny; j++) {
      ...
      for (i = 0; i <= mbnx; i++) {
        ...
        g_pPotGrid[index] +=
            MB_METABALLS * MB_METABALLS /
                ((x - tx1) * (x - tx1) + (y + ty1) * (y + ty1) +
                 (z - tz1) * (z - tz1)) +
            MB_METABALLS * MB_METABALLS /
                ((x - tx2) * (x - tx2) + (y + ty2) * (y + ty2) +
                 (z - tz2) * (z - tz2)) +
            MB_METABALLS * MB_METABALLS /
                ((x - tx3) * (x - tx3) + (y + ty3) * (y + ty3) +
                 (z - tz3) * (z - tz3)) +
            MB_METABALLS * MB_METABALLS /
                ((x - tx4) * (x - tx4) + (y + ty4) * (y + ty4) +
                 (z - tz4) * (z - tz4));
        x += deltax;
      }
      y += deltay;
...
void MetaballsInitMarchingCubes(float x1, float x2, float y1, float y2,
                                float z1, float z2, int m, int n, int p,
                                float pot) {
  int i, j, k;
  ...
  for (k = 0; k < mbnz; k++) {
    ...
    for (j = 0; j < mbny; j++) {
      ...
      for (i = 0; i < mbnx; i++) {
        ...
        if (((f1 >= mbk) && (f2 < mbk)) || ((f1 < mbk) && (f2 >= mbk))) {
          g_pVertexGrid[i + j * mbnx + k * mbnx_x_mbny + mbnx_x_mbny_x_mbnz] =
              g_dwVertices;
          g_pVertex[g_dwVertices].normal.x = (0.0f);
          g_pVertex[g_dwVertices].normal.y = (0.0f);
        ...
        if (((f1 >= mbk) && (f2 < mbk)) || ((f1 < mbk) && (f2 >= mbk))) {
          g_pVertexGrid[i + j * mbnx + k * mbnx_x_mbny + mbnx_x_mbny_x_mbnz +
                        mbnx_x_mbny_x_mbnz] = g_dwVertices;
          g_pVertex[g_dwVertices].normal.x = (0.0f);
          g_pVertex[g_dwVertices].normal.y = (0.0f);
  ...
  for (k = 0; k < mbnz - 1; k++) {
    ...
    for (j = 0; j < mbny - 1; j++) {
      ...
      for (i = 0; i < mbnx - 1; i++) {
        ...
        for (l = 0; l < 4; l++) {
          if (g_pMetaballTriangle[index + l * 3] !=
              g_pMetaballTriangle[index + l * 3 + 1]) {
            eindex = g_pMetaballTriangle[index + l * 3] * 4;
            g_pFace[g_dwFaces].x =
                (g_pVertexGrid[i + edge[eindex + 1] +
                               (j + edge[eindex + 2]) * mbnx +
                               (k + edge[eindex + 3]) * mbnx_x_mbny +
                               edge[eindex] * mbnx_x_mbny_x_mbnz]);
...
int init(logT log_function) { return 1; }
...
void update(void *instance) {
  ...
  for (int indexI = 0; indexI < WIDTH * HEIGHT; indexI++) {
    zBuffer[indexI] = -999999999.0f;
  }
  ...
  Matrix o2w;
  o2w.rotate(angle, angle, angle);

  // world 2 camera
  Matrix w2c;
  w2c.identity();
  w2c.scale(25.0, 25.0, 25.0);
  ...
  for (signed int i = 0; i < balls.numFaces; i++) {
    ...
    if ((Vector(0, 0, -1) % balls.faces[i].Normal) < 0) {
      int color =
          (int)(0xff * (1.0 - cos(Vector(0, 0, 1) % balls.faces[i].Normal)));
