...
static const char *SPEC = "mod_spec { name=mod_combineModule; "
...
int getInfo(char *buf, int bufLen) {
  static const char *INFO = "info { name=RGBCombine group=Mixer inputs=[3 Bild "
                            "Bild Bild] outputs=[1 Bild] type=xpm}";
  char *tmpBuf;
  ...
  if (buf != 0 && reqLen <= bufLen) {
    ...
    for (i = 0; i < lines; ++i) {
      const char *source = combine_xpm[i];
