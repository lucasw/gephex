...
public:
  ...
  ~udp_listen_socket() { close(sockfd); }

  bool add_package(std::vector<uint8_t> &p) {
    struct sockaddr_in cliaddr;
    socklen_t len = sizeof(cliaddr);

    int n = recvfrom(sockfd, &buffer[0], buffer.size(), 0, (sockaddr *)&cliaddr,
                     &len);
    if (n < 0) {
      ...
      std::copy(reinterpret_cast<uint8_t *>(&n),
                reinterpret_cast<uint8_t *>(&n) + 4, back_inserter(p));
