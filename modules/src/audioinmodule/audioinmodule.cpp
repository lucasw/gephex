...
static const AudioInDriver::sample_format SAMPLE_FORMAT =
    AudioInDriver::SF_16LE;
static const int BYTES_PER_SAMPLE = 2;
static const int CHANNELS = 1;

static const int AUDIO_BUFFER_SIZE =
    SAMPLE_RATE * BYTES_PER_SAMPLE * CHANNELS / 6;
...
#error No sound driver!
  ...
  if (device != my->device_number || !my->drv->is_open()) {
    ...
    try {
      my->drv->open(s_log, device, SAMPLE_RATE, SAMPLE_FORMAT, CHANNELS);
  ...
  try {
    len = my->drv->read(data, sizeof(data) / BYTES_PER_SAMPLE);
  } catch (std::exception &e) {
    s_log(0, e.what());
  }
