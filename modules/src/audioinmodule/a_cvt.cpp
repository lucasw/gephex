...
void a_cvt_16le_to_double_mono(const int16_t *src, double *dst,
                               int num_samples) {
  if (is_little_endian()) {
    std::transform(src, src + num_samples, dst, c_16le_to_double_le);
  } else {
    std::transform(src, src + num_samples, dst, c_16le_to_double_be);
  }
}

void a_cvt_double_to_16le_mono(const double *src, int16_t *dst,
                               int num_samples) {
  if (is_little_endian()) {
    std::transform(src, src + num_samples, dst, c_double_to_16le_le);
  } else {
    std::transform(src, src + num_samples, dst, c_double_to_16le_be);
  }
}
