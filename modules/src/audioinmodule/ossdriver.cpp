...
OSSDriver::OSSDriver() : m_impl(new Impl()) {}
...
void OSSDriver::open(logT /*logger*/, device_id_t device, int sample_rate,
...
int OSSDriver::read(unsigned char *data, int num_samples) {
  ...
  int len = read_data(m_impl->fd, data, bytes_to_read);
...
bool OSSDriver::is_open() const { return m_impl->fd != -1; }
