...
/**
 * See
 * http://developer.apple.com/documentation/MusicAudio/Reference/CACoreAudioReference/index.html
 */
...
struct CoreAudioDriver::Impl {
  ...
  void new_data(const unsigned char *data, UInt32 numBytes) {
    ...
    const size_t MAX_BUFFER = static_cast<size_t>(
        (m_format.mBytesPerFrame * m_format.mSampleRate) / 10);
  ...
  int read(unsigned char *data, int num_samples) {
    ...
    const UInt32 inputDataSizeBytes =
        my_min(num_samples * m_format.mBytesPerFrame, m_buffer.size());
  ...
  static OSStatus InputIOProc(AudioDeviceID inDevice,
    ...
    impl->new_data(
        reinterpret_cast<const unsigned char *>(inputData->mBuffers[0].mData),
        inputData->mBuffers[0].mDataByteSize);
...
CoreAudioDriver::CoreAudioDriver() : m_impl(new Impl()) {}
...
void CoreAudioDriver::open(logT logger, device_id_t device, int sample_rate,
                           sample_format form, int channels) {
  ...
  m_impl->m_desired_format.mBytesPerFrame =
      m_impl->m_desired_format.mBytesPerPacket;
...
#define USE_COMPLEX_AUDIO_CONVERTER
  ...
  m_impl->m_converter =
      new SimpleConverter(m_impl->m_format, m_impl->m_desired_format);
#elif defined(USE_COMPLEX_AUDIO_CONVERTER)
  m_impl->m_converter =
      new ComplexConverter(m_impl->m_format, m_impl->m_desired_format);
