...
WaveInDriver::WaveInDriver() : m_impl(new Impl()) {}
...
void WaveInDriver::open(logT /*logger*/, device_id_t device, int sample_rate,
  ...
  m_impl->buf =
      ab_create(m_impl->handle, CHUNK_SIZE * bytes_per_sample, NUM_CHUNKS);
...
int WaveInDriver::read(unsigned char *data, int num_samples) {
  ...
  while (ab_block_ready(m_impl->buf) &&
    ...
    samples_read += ab_get_block(m_impl->buf, data + samples_read * 2,
                                 num_samples * 2 - samples_read * 2) /
                    2;
  }
  return samples_read;
}

bool WaveInDriver::is_open() const { return m_impl->handle != 0; }
