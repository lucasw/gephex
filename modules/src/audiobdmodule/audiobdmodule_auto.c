...
{
   if (s_log_function)
      s_log_function(level, "mod_audiobdmodule", msg);
}

const char* getSpec(void) {
 return "mod_spec { name=[mod_audiobdmodule] number_of_inputs=[3] number_of_outputs=[1] deterministic=[true] }";
}
const char* getInputSpec(int index) {
 switch(index) {
  ...
  break;
  case 1:
    return "input_spec { type=typ_NumberType const=true strong_dependency=true default=0.3 } ";
  break;
  case 2:
    return "input_spec { type=typ_StringType const=true strong_dependency=true default=exp } ";
  break;
 }
 return 0;
}
  ...
  if (buf != 0 && reqLen <= bufLen)
    {
      char* offset;
      int i;
      int lines = getNumberOfStringsXPM(audiobdmodule_xpm);
      ...
      for (i = 0; i < lines; ++i)
	{
	  char* source = audiobdmodule_xpm[i];
