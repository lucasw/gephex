...
FFMpegWriter::FFMpegWriter(const std::string &filename, int xres, int yres,
  ...
  std::cout << "creating new ffmpeg writer " << filename << " " << xres << "x"
            << yres << ", encoding: " << encoding << std::endl;
...
void FFMpegWriter::writeFrame(uint32_t *frame) {
  ...
  avpicture_fill((AVPicture *)frameRGB, (uint8_t *)frame, PIX_FMT_RGBA32, x_res,
                 y_res);
...
std::string FFMpegWriter::getFilename() { return outfile; }

int FFMpegWriter::getXres() { return x_res; }

int FFMpegWriter::getYres() { return y_res; }

std::string FFMpegWriter::getEncoding() { return encstr; }
...
void FFMpegWriter::write_video_frame(AVFormatContext *oc, AVStream *st,
  ...
  img_convert((AVPicture *)picture, c->pix_fmt, (AVPicture *)picture_ptr,
              PIX_FMT_RGBA32, c->width, c->height);
  ...
  if (oc->oformat->flags & AVFMT_RAWPICTURE) {
    ...
    /* encode the image */
    out_size =
        avcodec_encode_video(c, video_outbuf, video_outbuf_size, final_picture);
