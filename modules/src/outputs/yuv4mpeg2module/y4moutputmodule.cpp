...
struct stream_dsc {
  stream_dsc() : color_model(NONE), fd(0) {
...
void destruct(MyInstance *my) { delete my; }
...
void update(void *instance) {
  ...
  try {
    ...
    // new stream?
    if (stream.fd == 0 || filename != stream.filename ||
        xsize != stream.x_res || ysize != stream.y_res ||
      ...
      case YUV420P:
        ...
        break;
      case BGRA8888:
        stream_header_str << "BGRA8888 ";
        break;
      default:
        assert(!"invalid color model");
      }
      ...
      stream.yuv_buf.resize(
          stream.stream_header_size + stream.frame_header_size +
          get_frame_size(stream.x_res, stream.y_res, stream.color_model));

      // stream_header
      std::copy(stream_header.begin(), stream_header.end(),
                stream.yuv_buf.begin());

      // frame_header
      std::copy(frame_header.begin(), frame_header.end(),
    ...
    uint8_t *buf =
        &stream.yuv_buf[stream.stream_header_size + stream.frame_header_size];
    ...
    if (stream.color_model == YUV420P) {
      ...
      cvt_rgb32_to_i420(buf, fb, stream.x_res, stream.y_res, pitches, offsets);
    } else if (stream.color_model == BGRA8888) {
      ls_scale32((uint32_t *)buf, stream.x_res, stream.y_res,
                 inst->in_in->framebuffer, inst->in_in->xsize,
                 inst->in_in->ysize);
    }
