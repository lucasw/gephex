...
static __inline void lfo_deleteInstance(LfoType *lfo) { free(lfo); }

static __inline int lfo_serialize(const LfoType *lfo, char *buffer,
                                  int bufferLen) {
...
static __inline void lfo_deSerialize(const char *buffer, int len,
                                     LfoType *lfo) {
  if (buffer[0] == 0) {
