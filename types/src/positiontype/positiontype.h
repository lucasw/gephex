...
static __inline void pos_deleteInstance(PositionType *pos) { free(pos); }

static __inline int pos_serialize(const PositionType *pos, char *buffer,
                                  int bufferLen) {
...
static __inline void pos_deSerialize(const char *buffer, int len,
                                     PositionType *pos) {
  if (buffer[0] == 0) {
