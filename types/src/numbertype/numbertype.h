...
static __inline void number_deleteInstance(NumberType *num) { free(num); }

static __inline int number_serialize(const NumberType *number, char *buffer,
                                     int bufferLen) {
...
static __inline int trim_bool(double val) { return trim_int(val, 0, 1) != 0; }

static __inline double trim_double(double val, double low_val,
                                   double high_val) {
  if (val < low_val)
    return low_val;
