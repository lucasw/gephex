...
typedef struct monitor_ {
  unsigned char r, g, b;
  int showsPic; // true, if this monitor is supposed to just display the input
                // signal
} Monitor;
...
videowallcontrol_initInstance(VideoWallControlType *newType) {
  int i, j;
...
static __inline VideoWallControlType *videowallcontrol_newInstance(void) {
  VideoWallControlType *newType =
      (VideoWallControlType *)malloc(sizeof(VideoWallControlType));
  videowallcontrol_initInstance(newType);
  return newType;
}
static __inline void videowallcontrol_assign(VideoWallControlType *dst,
                                             const VideoWallControlType *src) {
  memcpy(&dst->monitors, &src->monitors, sizeof(Monitor) * NUM_MONITORS);
}

static __inline void
videowallcontrol_deleteInstance(VideoWallControlType *vwc) {
  free(vwc);
}

static __inline int
videowallcontrol_serialize(VideoWallControlType *videowallcontrol, char *buffer,
                           int bufferLen) {
  ...
  // Let's say for now we juist print the vals from left to right and top to
  // bottom that makes uhm... 36*(2*3+4)+1(showpic should only be 0 or 1,
  // r,g,b>=0<=63) +1 for 0   TODO
  int x, y, base = 0;
    ...
    for (x = 0; x < NUM_MONITORS; ++x) {
      for (y = 0; y < NUM_MONITORS; ++y) {
        if (videowallcontrol->monitors[x][y].showsPic == 1) {
          ...
          base += 16;
          if (videowallcontrol->monitors[x][y].r >= 10)
            ++base;
          if (videowallcontrol->monitors[x][y].g >= 10)
            ++base;
          if (videowallcontrol->monitors[x][y].b >= 10)
            ++base;
        }
      }
    }
...
static __inline void
videowallcontrol_deSerialize(const char *buffer, int len,
                             VideoWallControlType *videowallcontrol) {
  int x = 0, y = 0, base = 0;
  ...
  if (buffer[0] == 0) {
    ...
    /*for(x=0; x<NUM_MONITORS; ++x)
    {
            for(y=0; y<NUM_MONITORS; ++y)
            {
                    ...
                    sscanf(buffer+base, "%u %u %u %u ",
    &videowallcontrol->monitors[i].r, &videowallcontrol->monitors[i].g,
                            &videowallcontrol->monitors[i].b,
    &videowallcontrol->monitors[i].showsPic); base+=10;
            }
    }*/
    free(command);
  }
