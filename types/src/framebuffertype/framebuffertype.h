...
static __inline FrameBufferType *framebuffer_newInstance(void) {
  FrameBufferType *newType = (FrameBufferType *)malloc(sizeof(FrameBufferType));
...
static __inline void framebuffer_destroyInstance(FrameBufferType *fbuffer) {
  if (fbuffer->data)
    free(fbuffer->data);
...
static __inline void framebuffer_assign(FrameBufferType *dst,
  ...
  memcpy(dst->framebuffer, src->framebuffer,
         src->xsize * src->ysize * sizeof(uint_32));
}

static __inline int framebuffer_serialize(const void *type, char *buffer,
                                          int bufferLen) {
...
static __inline void framebuffer_deSerialize(const char *buffer, int len,
                                             void *type) {}

// scale src to dst
// returns 1 on success
static __inline int framebuffer_scale(FrameBufferType *dst,
                                      const FrameBufferType *src, int w,
                                      int h) {
  int y;
  uint_32 x_a, y_a;
  uint_32 A, B;
...
static __inline void framebuffer_swap(FrameBufferType *fb1,
                                      FrameBufferType *fb2) {
...
static __inline int
framebuffer_attributesEqual(FrameBufferType *type,
                            FrameBufferAttributes *attributes) {
  return (attributes->xsize == type->xsize && attributes->ysize == type->ysize);
}

// returns 1 on success
static __inline int framebuffer_convertType(FrameBufferType *dstType,
                                            const FrameBufferType *srcType,
                                            FrameBufferAttributes *attributes) {
  return framebuffer_scale(dstType, srcType, attributes->xsize,
                           attributes->ysize);
}

// returns 1 on success
static __inline int
framebuffer_changeAttributes(FrameBufferType *fb,
                             FrameBufferAttributes *attributes) {
...
static __inline void framebuffer_set_default(FrameBufferType *fb) {
