...
static __inline void env_deleteInstance(EnvType *env) { free(env); }

static __inline int env_serialize(const EnvType *env, char *buffer,
                                  int bufferLen) {
  if (buffer == 0 || bufferLen < 50)
    return 50;
  else {
    sprintf(buffer, "%g %g %g %g %g", env->amount, env->attack, env->decay,
            env->sustain_level, env->release);
    return strlen(buffer) + 1;
  }
}

static __inline void env_deSerialize(const char *buffer, int len,
                                     EnvType *env) {
  if (buffer[0] == 0) {
    env_initInstance(env);
  } else {
    sscanf(buffer, "%lf %lf %lf %lf %lf", &env->amount, &env->attack,
           &env->decay, &env->sustain_level, &env->release);
  }
}
