...
Protocol::Protocol()
    : m_sender(0), m_listener(0), bufferBegin(new unsigned char[BUFFER_SIZE]),
      bufferPos(bufferBegin), bufferEnd(bufferBegin + BUFFER_SIZE),
      m_silent_mode(false) {}

Protocol::Protocol(IDataListener &dl)
    : m_sender(0), m_listener(&dl), bufferBegin(new unsigned char[BUFFER_SIZE]),
      bufferPos(bufferBegin), bufferEnd(bufferBegin + BUFFER_SIZE),
      m_silent_mode(false) {}

Protocol::~Protocol() { delete[] bufferBegin; }

void Protocol::registerListener(IDataListener &listener) {
  m_listener = &listener;
}

void Protocol::registerSender(ISender *sender) { m_sender = sender; }
...
struct Header {
  explicit Header(int data_len)
      : packetLen(data_len + sizeof(Header)), version(PROT_VERSION),
        dataLen(data_len), checkSum(check_sum(packetLen, version, data_len)) {}
   ...
   */
  bool ok() const {
    return (version == PROT_VERSION && packetLen <= MAX_PACKET_LEN &&
...
void Protocol::read(const unsigned char *&dataPos, const unsigned char *dataEnd,
...
void Protocol::dataReceived(const utils::Buffer &buf) {
  ...
  while (dataPos < dataEnd) // noch weitere daten da
  {
    ...
    if (bufferPos < bufferBegin + sizeof(Header)) {
      ...
      if (bufferPos - bufferBegin + bytesLeft <
          (int)sizeof(Header)) { // kann header nicht fertig lesen
    ...
    } else // nach header
    {
      ...
      if (!header.ok()) {
        ...
        fprintf(stderr, "pl = %i, dl = %i\n", header.packetLen, header.dataLen);
        ...
        if (syncPoint == -1) {
          fprintf(stderr, "Skipping whole packet (%li bytes)\n",
                  dataEnd - dataPos);
      ...
      if (static_cast<uint_32>(bufferPos - bufferBegin + bytesLeft) <
          header.packetLen) { // Paket nicht ganz da
      ...
      } else                        // ganzes Paket da
      {
        ...
        if (header.version == PROT_VERSION && m_listener != 0) {
          try {
            m_listener->dataReceived(
                utils::Buffer(bufferBegin + sizeof(Header),
...
void Protocol::reset() { bufferPos = bufferBegin; }
