...
void StringTagUtil::attachTag(utils::Buffer &dst, const std::string &tag,
...
void StringTagUtil::removeTag(utils::Buffer &dst, std::string &tag,
  ...
  tag = std::string(reinterpret_cast<const char *>(data + sizeof(uint32_t)),
                    l - 1);
...
void UInt32TagUtil::attachTag(utils::Buffer &dst, const uint_32 &tag,
                              const utils::Buffer &b) {
...
void UInt32TagUtil::removeTag(utils::Buffer &dst, uint_32 &tag,
                              const utils::Buffer &src) {
  const unsigned char *data = src.getPtr();
  tag = convert_uint32_from_le(data);

  dst = utils::Buffer(data + sizeof(uint_32), src.getLen() - sizeof(uint_32));
