...
public:
  ...
  void setListener(net::IDataListener &l) { m_listener = &l; }

  virtual ~TestSender() { delete[] m_buf; }
...
public:
  ...
  virtual ~TestInterpreter() { delete[] m_data; }

  virtual void dataReceived(const Tag &tag, const utils::Buffer &buf) {
    if (m_data != 0)
      delete[] m_data;
...
std::string TaggerTest::getName() const { return "Tagger"; }

std::string TaggerTest::getDescription() const { return ""; }

template <class Tag, class TagUtil> static void test_tagutil(const Tag &t) {
  TagUtil u;
  utils::Buffer b1;
  utils::Buffer b2;
...
template <class Tag, class TagUtil> static void test_tagger(const Tag &t) {
