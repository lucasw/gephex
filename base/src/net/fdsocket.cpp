...
/*const throw (IOException,UnknownHostException)*/
{
  ...
  int sent = 0;
  do {
    len = ::send(m_fd, reinterpret_cast<const char *>(buf), left, SEND_FLAGS);
    if (len > 0) {
      sent += len;
      left -= len;
      buf += len;
...
bool FDSocket::receive(utils::Buffer &b) throw(IOException) {
  ...
  if (m_timeout == -1 || !SocketUtils::doesReadBlock(m_fd, m_timeout)) {
    ...
    errno = 0;
    do {
      len = recv(m_fd, reinterpret_cast<char *>(buf), MAX_MSG_LEN, 0);
...
bool FDSocket::connected() const { return m_isConnected; }
