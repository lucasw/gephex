...
static int release_handle(void *h) { return DisconnectNamedPipe(h); }
...
NamedPipeServerSocket::~NamedPipeServerSocket() { delete m_impl; }
...
ISocket *NamedPipeServerSocket::accept() throw(std::runtime_error) {
  ...
  if (error == 0) {
    ...
    case 0:
      // std::cout << "... success!!!" << std::endl;
      return new FHSocket(m_impl->m_handle, release_handle,
                          m_impl->m_localPort);
