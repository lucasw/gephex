...
TCPClientSocket::TCPClientSocket(int localPort)
  ...
  if (bind(m_socket, (struct sockaddr *)&local_sin, sizeof(local_sin)) ==
      SOCKET_ERROR) {
...
ISocket *TCPClientSocket::connect(const std::string &hostName,
                                  int portNum) throw(std::runtime_error) {
  if (m_socket == INVALID_SOCKET) {
  ...
  if (phost == 0) {
    throw UnknownHostException("Could not resolve hostname '" + hostName + "'");
  }
  ...
  if (::connect(m_socket, (struct sockaddr *)&dest_sin, sizeof(dest_sin)) !=
      0) {
    return 0;
  }
