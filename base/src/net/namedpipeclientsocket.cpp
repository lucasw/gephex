...
static int release_handle(void *h) { return CloseHandle(h); }
...
ISocket *NamedPipeClientSocket::connect(const std::string &hostName,
  ...
  m_handle = CreateFile(name.c_str(), GENERIC_READ | GENERIC_WRITE, 0, 0,
                        OPEN_EXISTING, 0, 0);
