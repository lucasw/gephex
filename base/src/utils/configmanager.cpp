...
static void print_param_value(std::ostream &s, config_param_t::ParamValue val,
...
static void check_type(const utils::StructReader &sr, const std::string &name,
                       config_param_t::ParamType type) {
  switch (type) {
  case config_param_t::BOOL_PARAM:
    sr.getBoolValue(name);
    break;
  case config_param_t::INT_PARAM:
    sr.getIntValue(name);
    break;
  case config_param_t::STRING_PARAM:
    sr.getStringValue(name);
    break;
  }
}
...
static void create_cmdl_config(std::ostream &os,
                               const ConfigManager::ParamList &params, int argc,
                               const char *argv[], bool &help_requested) {
  os << "cmdl {\n";
...
static void check_cf_cmdl(const ConfigManager::ParamList & /*params*/,
  ...
  for (utils::StructReader::ConfigMap::const_iterator it = m.begin();
    ...
    if (cit == cpm.end())
      throw std::runtime_error("Unknown cmdl parameter: '--" + param_name +
                               "'");
...
config_param_t::config_param_t(const std::string &name, ParamType type,
                               ...
                               const std::string &description, ParamValue *def)
...
ConfigManager::ConfigManager(const std::string &cfile_name, int argc,
                             const char *argv[], const ParamList &params)
    : m_params(params), m_cf_file(0), m_cf_cmdl(0), m_help_requested(false) {
  for (ConfigManager::ParamList::const_iterator it = params.begin();
       it != params.end(); ++it) {
    if (m_cpm.count(it->m_name) != 0)
      throw std::runtime_error("Duplicate parameter name: '" + it->m_name +
                               "'");
...
bool ConfigManager::help_requested() const { return m_help_requested; }
...
std::string
ConfigManager::get_string_param(const std::string &param_name) const {
  CPMap::const_iterator it = m_cpm.find(param_name);
  if (it == m_cpm.end())
    throw std::runtime_error("Invalid param '" + param_name + "'");
