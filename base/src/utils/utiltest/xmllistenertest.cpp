...
XMLListenerTest::XMLListenerTest() : EasyTest("XMLListenerTest", "") {}
...
void XMLListenerTest::run() throw(std::runtime_error) {
  ...
  ModuleInfo *mi =
      new ModuleInfo("mod_intparserModule", "IntegerParser", ins, outs, true);
  ...
  ins2[0] = inpair;
  ModuleInfo *mi2 =
      new ModuleInfo("mod_glOutputModule", "GLoutput", ins, outs, true);
  ...
  XMLTokenListener horcher(mi_map, infos, nodes, nodeFileIDs, controls,
                           controlFileIDs, conns, ctrlconns);
