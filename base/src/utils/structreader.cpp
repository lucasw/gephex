...
public:
  ...
  virtual void scannedStructName(const std::string &name) { _name = name; }
...
StructReader::StructReader(const ConfigMap &map) : m_values(map) {}
...
bool StructReader::getBoolValue(const std::string &name,
...
std::string
StructReader::getStringValue(const std::string &name,
...
StructReader::ConfigMap StructReader::getMap() const { return m_values; }

void StructReader::serialize(std::ostream &out) const {
  out << m_name << " { ";
  for (ConfigMap::const_iterator it = m_values.begin(); it != m_values.end();
       ++it) {
    out << it->first;
    out << "=[" << it->second << "]; ";
  }
