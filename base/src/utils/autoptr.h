...
public:
   ...
   */
  AutoPtr(const AutoPtr<T> &ap) : m_ref(ap.m_ref), m_ptr(ap.m_ptr) {
    m_ref->incRefCount();
  }
...
#if !defined(_MSC_VER)
  template <typename U>
  AutoPtr<T>(const AutoPtr<U> &ap) : m_ref(ap.ref()), m_ptr(ap.c_ptr()) {
    m_ref->incRefCount();
  }

  template <typename U> AutoPtr<T> &operator=(const AutoPtr<U> &ap) {
    if (m_ptr != ap.c_ptr()) {
      decRefCount();
      m_ptr = ap.c_ptr();
      m_ref = ap.ref();
   ...
   */
  T *c_ptr() const { return m_ptr; }

  /**
   * Internal Function
   */
  AutoPtrRef *ref() const { return m_ref; }
  ...
  /**
       ...
       * @return true if this points to the same Object as ap
                         false otherwise
       */
  bool operator==(const AutoPtr<T> &ap) const { return m_ptr == ap.m_ptr; }
   ...
   * @return false if this points to the same Object as ap
                     true otherwise
   */
  bool operator!=(const AutoPtr<T> &ap) const { return !(*this == ap); }
...
template <typename T>
AutoPtr<T>::AutoPtr(T *p) : m_ref(new AutoPtrRef()), m_ptr(p) {}

template <typename T> AutoPtr<T>::~AutoPtr() {
  if (m_ref != 0)
    decRefCount();
}
