...
BufferStreamTest::BufferStreamTest() : EasyTest("BufferStreamTest", "") {}
...
void BufferStreamTest::run() throw(std::runtime_error) {
...

  {
    ...
    obs << 5 << ' ' << utils::String("Das ist ein Test.") << ' ' << 1 << ' '
        << 11;
    obs.flush();
    ...
    if (test != utils::String("Das ist ein Test.") || i != 5 || j != 1 ||
        k != 11) {
      std::ostringstream msg;
      msg << "fehler: <" << test << "> != <Das ist ein Test.> oder " << i
          << " != 5 oder " << j << " != 1 oder " << k << " != 11.";
      throw std::runtime_error(msg.str().c_str());
    }
