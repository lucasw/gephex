...
std::string XmlutilsTest::getName() const { return "XmlutilsTest"; }

std::string XmlutilsTest::getDescription() const { return ""; }
...
void test3() {
  ...
  std::string tag =
      utils::xml::next_tag_name(test_text.begin(), test_text.end());

  if (tag != "bumm")
    throw std::runtime_error(
        (std::string("tag is '") + tag + std::string(" but should be 'bumm'"))
            .c_str());
}
