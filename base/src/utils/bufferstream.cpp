...
Buffer_streambuf::Buffer_streambuf(Buffer &buf) : _buf(buf) {
  initInOutBuffer(0);
}
...
Buffer_streambuf::int_type Buffer_streambuf::overflow(int_type c) {
  sync();
  if (c != EOF) {
    *_cache = c;
    pbump(1);
...
IBufferStream::~IBufferStream() { delete rdbuf(); }

OBufferStream::OBufferStream(Buffer &b) : inherited(new Buffer_streambuf(b)) {}

OBufferStream::~OBufferStream() { delete rdbuf(); }
} // namespace utils
