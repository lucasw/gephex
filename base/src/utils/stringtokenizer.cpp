...
std::string StringTokenizer::next(const std::string &delim) {
  ...
  if (m_pos != std::string::npos) {
    ...
    std::string::size_type first =
        m_input.find_first_not_of(delim.c_str(), m_pos);

    if (first != std::string::npos) {
      // length of found token
      std::string::size_type num =
          m_input.find_first_of(delim.c_str(), first) - first;
...
std::string::size_type StringTokenizer::getPos() const { return m_pos; }

const std::string &StringTokenizer::getString() const { return m_input; }
