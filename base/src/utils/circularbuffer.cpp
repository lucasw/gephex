...
CircularBuffer::CircularBuffer()
    : m_capacity(INITIAL_CAPACITY), m_begin(0), m_end(0),
      m_data(new unsigned char[m_capacity]) {}

CircularBuffer::~CircularBuffer() { delete[] m_data; }
...
void CircularBuffer::push_back(const utils::Buffer &buf) {
  ...
  if (new_len >= m_capacity) {
    int needed_capacity =
        max_(static_cast<int>(m_capacity * GROW_FACTOR), new_len) + 1;
...
void CircularBuffer::clear() { m_begin = m_end = 0; }
} // namespace utils
