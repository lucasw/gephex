...
Buffer::Buffer(const unsigned char *b, int l, int maxLengthHint)
    : m_len(l), m_capacity(max(l, maxLengthHint)) {
  m_buffer = new unsigned char[m_capacity];
  memcpy(m_buffer, b, m_len);
}

Buffer::Buffer(unsigned char *b, int l, bool owner, int maxLengthHint)
    : m_len(l), m_capacity(max(l, maxLengthHint)) {
  if (!owner) {
    m_buffer = new unsigned char[m_capacity];
    memcpy(m_buffer, b, m_len);
...
Buffer::Buffer(const Buffer &b, int maxLengthHint)
    : m_buffer(new unsigned char[max(b.getLen(), maxLengthHint)]),
      m_len(b.getLen()), m_capacity(max(b.getLen(), maxLengthHint)) {
  memcpy(m_buffer, b.getPtr(), m_len);
}
...
const unsigned char *Buffer::getPtr() const { return m_buffer; }

int Buffer::getLen() const { return m_len; }
