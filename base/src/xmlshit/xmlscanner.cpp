...
void XMLFileScanner::scanInfoSection(std::string &text) {
  ...
  // TODO derzeit wird nur name des moduls eingelesen...müsste aber eigentlich
  // reichen
  ...
  while (token != "/moduleinfos") {
    if (token != "moduleinfo")
      throw std::runtime_error(
          "Error in Format in moduleinfo section discovered..");
    ...
    while (token != "/moduleinfo") {
      token = st.next(delim);
      if (token == "moduleinfo")
        throw std::runtime_error("Error in Format in modulinfo section "
                                 "discovered...closing tag missing...");
    }
...
void XMLFileScanner::scanNodeSection(std::string &text) {
  ...
  while (token != "/nodes") {
    ...
    while (token != "/node") {
      ...
      if (token == "node")
        throw std::runtime_error(
            "Wrong Format in Node Section discoverd..closing tag missing\n");
...
void XMLFileScanner::scanConnectionSection(std::string &text) {
  ...
  while (token != "/connections") {
    if (token != "connection")
      throw std::runtime_error(
          "Wrong Format in connection Section discoverd..\n");

    while (token != "/connection") {
      token = st.next(delim);
      if (token == "connection")
        throw std::runtime_error("Wrong Format in connection Section "
                                 "discoverd..closing tag missing\n");
...
void XMLFileScanner::scanCtrlSection(std::string &text) {
  ...
  while (token != "/controls") {
    ...
    while (token != "/control") {
      ...
      if (token == "control")
        throw std::runtime_error(
            "Wrong Format in control Section discoverd..closing tag missing\n");
...
void XMLFileScanner::scan(std::string &text) {
  ...
  if (token == "model") {
    ...
    throw std::runtime_error(
        "This is no Ge-Phex Graph file...go fuck your dog!!!");
  }
}
