...
XMLFileReader::XMLFileReader(const std::map<int, ModuleInfo *> &mi,
                             std::ifstream &file) {
  XMLTokenListener listener(mi, infos, nodes, controls, connections,
                            controlInfos);
...
/*int XMLFileReader::getRealNodeIDByFileID(int pos)
{
        ...
        while(j!=nodeFileIDs.end())
        {
                if(pos == j->first)
                {
                        ...
                        throw std::runtime_error("name not in infos??? contact
your local suicide box...");
...
/* Die Connnection Ids im file werden nach position vergeben, hier kriegt man
 * daraus den namen */
