...
XMLTokenListener::XMLTokenListener(
    const std::map<int, ModuleInfo *> &infoMap_,
    std::map<std::string, int> &infos_, std::list<NodeInfo> &nodes_,
    std::list<NodeInfo> &controls_,
    std::map<std::pair<int, int>, std::pair<int, int>> &conns,
    std::list<ControlInfo> &ctrlconns)
    : infoMap(infoMap_), infos(infos_), nodes(nodes_), controls(controls_),
      connections(conns), ctrlInfos(ctrlconns) {}
...
void XMLTokenListener::scannedModuleInfo(std::string &name) {
  ...
  throw std::runtime_error("Module not found...wether module \nhas changed "
                           "since last run or dll's not been loaded...\n");
}
...
#ifdef VERBOSE_XML_LISTENER
  std::cout << "received scannedNode: " << name << ", " << x << ", " << y
            << std::endl;
#endif
...
void XMLTokenListener::scannedConnection(std::string &connection) {
  ...
  s >> in;
#ifdef VERBOSE_XML_LISTENER
  std::cout << "received scannedConnection: " << id1 << ", " << out << ", "
            << id2 << ", " << in << std::endl;
#endif
