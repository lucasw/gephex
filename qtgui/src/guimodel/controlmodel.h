...
class ControlModel : public IModuleDataReceiver, public IGraphDataReceiver {
public:
  typedef std::map<std::string, std::string> ParamMap;
  ControlModel(IModelControlReceiver &, IErrorReceiver &log);
  virtual ~ControlModel();

  void registerControlView(IControlView &);
  void registerControlConnectView(IControlConnectView &);

  virtual void addControl(const Point &pos, const std::string &name, int nodeID,
  ...
  virtual void moduleDataSet(int moduleID, int type, const utils::Buffer &buf);
