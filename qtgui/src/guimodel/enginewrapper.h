...
namespace net {
template <typename Key, class Serializer> class Tagger;

template <typename Tag> class TagDispatcher;
...
class EngineWrapper {
public:
  EngineWrapper(const std::string &ipcType, const std::string &locator,
                int port, IErrorReceiver &log);
  ~EngineWrapper();
