...
void GraphModel::registerGraphView(IGraphView &_view) { view = &_view; }

void GraphModel::moveModule(int id, const Point &pos) {
  m_receiver->setModuleData(id, datatype::POINT, point2Buffer(pos));
}
...
void GraphModel::modulesConnected(int moduleID1, int outputIndex, int moduleID2,
                                  int inputIndex) {
...
void GraphModel::moduleDeleted(int moduleID) { view->moduleDeleted(moduleID); }
