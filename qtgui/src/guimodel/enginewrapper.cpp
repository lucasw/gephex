...
EngineWrapper::EngineWrapper(const std::string &ipcType,
    ...
    : m_ipcType(ipcType), m_locator(locator), m_port(port),
      ...
      mcisn(new ModuleClassInfoSenderNet()), mdsn(new ModuleDataSenderNet()),
      gdsn(new GraphDataSenderNet()), cvsn(new ControlValueSenderNet()),
      mssn(new ModuleStatisticsSenderNet()), gnsn(new GraphNameSenderNet()),
      esn(new ErrorSenderNet()), mstsn(new ModelStatusSenderNet()),
      rssn(new RendererStatusSenderNet()),

      tagger1(new CommandTagger(*mcdsn)), tagger2(new CommandTagger(*mcisn)),
      tagger3(new CommandTagger(*mdsn)), tagger4(new CommandTagger(*cvsn)),
      tagger5(new CommandTagger(*mssn)), tagger7(new CommandTagger(*gnsn)),
      tagger10(new CommandTagger(*esn)), tagger13(new CommandTagger(*mstsn)),
      tagger14(new CommandTagger(*rssn)), tagger15(new CommandTagger(*gdsn)),

      portTagger1(new PortTagger()), portTagger2(new PortTagger()),
      portTagger3(new PortTagger()), portTagger4(new PortTagger()),
      portTagger5(new PortTagger()), portTagger7(new PortTagger()),
      portTagger10(new PortTagger()), portTagger13(new PortTagger()),
      portTagger14(new PortTagger()), portTagger15(new PortTagger()),

      portDispatcher(new PortDispatcher()),
      portTagger(new PortTagger(*portDispatcher)),
      protocol(new net::Protocol(*portTagger)), socket(0),
      ...
      gModel(new GraphModel(*mcrn)), cModel(new ControlModel(*mcrn, log)),
  ...
  initTaggers(*portTagger13, *tagger13, *protocol, *portDispatcher,
              m_port + 12);
  initTaggers(*portTagger14, *tagger14, *protocol, *portDispatcher,
              m_port + 13);
  initTaggers(*portTagger15, *tagger15, *protocol, *portDispatcher,
              m_port + 14);
...
void EngineWrapper::connect() {
  ...
  if (socket == 0) {
    ...
    if (m_ipcType == "inet") {
      ...
      throw std::runtime_error("Unknown ipcType '" + m_ipcType +
                               "' at "
...
IControlValueSender &EngineWrapper::controlValueSender() { return *cvsn; }

IModuleStatisticsSender &EngineWrapper::moduleStatisticsSender() {
  return *mssn;
}

IGraphNameSender &EngineWrapper::graphNameSender() { return *gnsn; }

IErrorSender &EngineWrapper::errorSender() { return *esn; }

IModelStatusSender &EngineWrapper::modelStatusSender() { return *mstsn; }

IRendererStatusSender &EngineWrapper::rendererStatusSender() { return *rssn; }

IRendererControlReceiver &EngineWrapper::rendererControlReceiver() {
  return *rcrn;
}

IEngineControlReceiver &EngineWrapper::engineControlReceiver() { return *ecrn; }

IModelControlReceiver &EngineWrapper::modelControlReceiver() { return *mcrn; }

GraphModel &EngineWrapper::graphModel() { return *gModel; }

ControlModel &EngineWrapper::controlModel() { return *cModel; }

ModuleClassModel &EngineWrapper::moduleClassModel() { return *classModel; }
