...
namespace {
std::string mySubstr(const std::string &t, std::string::size_type pos1,
...
std::vector<PlugInfo> processArray(const std::string &s) {
  ...
  for (int i = 0; i < size; ++i) {
    try {
      ...
      std::cerr << "Error while processing <" << s << ">, i = " << i
                << std::endl;
      throw;
    }
  }
...
void ModuleClassModel::unregisterModuleClassViews() { views.clear(); }
