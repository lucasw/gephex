...
public:
  ...
  ControlElement(const std::string &name, const Point &, int _nodeID,
                 int _inputIndex, const std::string &_widgetType,
  ...
  std::string widgetType() const { return _widgetType; }

  int inputIndex() const { return _inputIndex; }

  const Point &position() const { return _pos; }

  void setPosition(const Point &p) { _pos = p; }

  ParamMap params() const { return _params; }

  const void setParams(const ParamMap &params) { _params = params; }
};

ControlElement::ControlElement(const std::string &name, const Point &pos,
...
ControlModel::ControlModel(IModelControlReceiver &r, IErrorReceiver &log)
    : lastControlID(1023), lastLabelID(1023), mcr(&r), m_log(log) {}

ControlModel::~ControlModel() {}

void ControlModel::registerControlView(IControlView &v) { view = &v; }
...
ControlElement buffer2CElem(const utils::Buffer &b) {
  ...
  std::string allText(reinterpret_cast<const char *>(b.getPtr()), b.getLen());
...
void ControlModel::addControl(const Point &pos, const std::string &name,
  ...
  mcr->setModuleData(nodeID, controlID, cElem2Buffer(temp));
}
...
void ControlModel::addLabel(const Point &pos, const std::string &text) {
  ...
  mcr->setEditGraphData(labelID, lElem2Buffer(temp));
}
...
void ControlModel::moduleDataSet(int /*moduleID*/, int type,
  ...
  if (i == controls.end()) {
    ...
    utils::AutoPtr<ControlElement> cElem(new ControlElement(buffer2CElem(buf)));
...
void ControlModel::syncDataStarted() { controls.clear(); }
...
void ControlModel::graphDataSet(int type, const utils::Buffer &buf) {
  ...
  if (i == labels.end()) {
    ...
    utils::AutoPtr<LabelElement> lElem(new LabelElement(buffer2LElem(buf)));
...
void ControlModel::graphDataUnSet(int type) {
  ...
  try {
    ...
    txt << "Could not delete label at ControlModel::"
        << "graphDataUnSet(): '" << e.what() << "'";
    m_log.error(txt.str());
  }
}

void ControlModel::syncGraphDataStarted() { labels.clear(); }
