...
public:
  ...
  void set_client(IErrorReceiver *client) { m_client = client; }
...
VJMainWindow::VJMainWindow(QWidget *parent, const utils::ConfigManager &config,
                           const std::string &ipc_locator,
                           const std::string &conf_base_dir)

    : QMainWindow(parent), m_error_proxy(new ProxyErrorReceiver()),
      engineWrapper(
          new EngineWrapper(config.get_string_param("ipc_type"), ipc_locator,
                            config.get_int_param("ipc_port"), *m_error_proxy)),
      running(false), connected(false), moduleClassView(0), graphNameView(0),
      m_config(config), m_kbManager(0), m_conf_base_dir(conf_base_dir) {
  if (false) {
  ...
  switcher = new PicSwitch(statusBar(), roter_mann, gruener_mann);
...
void VJMainWindow::closeEvent(QCloseEvent *event) { event->accept(); }

namespace {
QPoint pmax(const QPoint &a, const QPoint &b) {
  return QPoint(a.x() >= b.x() ? a.x() : b.x(), a.y() >= b.y() ? a.y() : b.y());
}
} // namespace
...
void VJMainWindow::quitSlot() { emit quitSignal(); }
...
void VJMainWindow::createActions() {
  ...
  connect(rendererStateAction, SIGNAL(triggered()), this,
          SLOT(setRendererState()));
  ...
  connectToEngineAction->setEnabled(false);
  connect(connectToEngineAction, SIGNAL(triggered()), this,
          SLOT(connectToEngine()));
  ...
  disConnectToEngineAction->setEnabled(false);
  connect(disConnectToEngineAction, SIGNAL(triggered()), this,
          SLOT(disconnectFromEngine()));
  ...
  connect(synchronizeEngineAction, SIGNAL(triggered()), this,
          SLOT(synchronize()));
  ...
  shutDownEngineAction->setEnabled(false);
  connect(shutDownEngineAction, SIGNAL(triggered()), this, SLOT(shutDown()));
...
void VJMainWindow::unbuildModuleBar() {
  ...
  // engineWrapper->moduleClassNameSender().unregisterModuleClassNameReceiver();
  // //TODO der wird beim naechsten register automatisch ueberschrieben
...
void VJMainWindow::buildSkeleton() {
  ...
  editorWidget = new EditorWidget(
      ...
      engineWrapper->modelStatusSender(), &*m_kbManager, *logWindow,
  ...
  connect(editorWidget, SIGNAL(undisplayProperties()), this,
          SLOT(undisplayProperties()));

  connect(editorWidget,
          SIGNAL(newEditGraph(const std::string &, const std::string &)),
  ...
  connect(graphNameView->signalObject(), SIGNAL(undisplayProperties()), this,
          SLOT(undisplayProperties()));
...
void VJMainWindow::connectToEngine() {
  try {
    ...
    try {
      ...
      std::string binary_name = m_config.get_string_param("engine_binary");
...
void VJMainWindow::startStop() { setRendererState(); }
...
void VJMainWindow::synchronize() { engineWrapper->synchronize(); }
