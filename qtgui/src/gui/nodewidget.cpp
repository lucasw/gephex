...
public:
  FixedValueKeyListener(IModelControlReceiver &model, int moduleID,
                        int inputIndex, const utils::Buffer &inputValue)
...
public:
  ToggleValueKeyListener(IModelControlReceiver &model, int moduleID,
                         int inputIndex, const utils::Buffer &inputValue1,
                         const utils::Buffer &inputValue2)

      : m_model(model), m_moduleID(moduleID), m_inputIndex(inputIndex),
        m_inputValue1(inputValue1), m_inputValue2(inputValue2), m_state(true) {}
...
static void insertKeyListeners(KeyListenerList &ll, const std::string &keys,
                               const std::string &toggle_keys,
                               KeyboardManager &kbManager, int moduleID,
                               int inputIndex, IModelControlReceiver &model,
                               IErrorReceiver &log) {
  ...
  while (token != "") {
    ...
    gui::Key k(keyName);
    utils::Buffer value(
        reinterpret_cast<const unsigned char *>(keyValue.c_str()),
        keyValue.length() + 1);

    utils::AutoPtr<IKeyListener> kl(
        new FixedValueKeyListener(model, moduleID, inputIndex, value));
  ...
  while (token != "") {
    ...
    try {
      gui::Key k(keyName);
      utils::Buffer value1(
          reinterpret_cast<const unsigned char *>(keyValue1.c_str()),
          keyValue1.length() + 1);

      utils::Buffer value2(
          reinterpret_cast<const unsigned char *>(keyValue2.c_str()),
          keyValue2.length() + 1);

      utils::AutoPtr<IKeyListener> kl(new ToggleValueKeyListener(
          model, moduleID, inputIndex, value1, value2));
...
NodeWidget::NodeWidget(QWidget *parent, Qt::WFlags fl, int _id,
                       const ModuleInfo &_info, const PicManager &picz,
                       const utils::AutoPtr<ControlValueDispatcher> &dispatcher,
                       IModelControlReceiver &mcr, KeyboardManager *kbManager,
                       IErrorReceiver &log, const std::string &media_path)
  ...
  for (unsigned int i = 0; i < _info.getInputs().size(); ++i) {
    ...
    if (m_kbManager) {
      ...
      insertKeyListeners(m_keyListeners, keys, toggle_keys, *m_kbManager, id, i,
                         mcr, m_log);
    }
    ...
    connect(&*inputs[i],
            SIGNAL(connectionRequestFromInput(const InputPlugWidget *,
                                              const QPoint &)),
            this,
            SLOT(connectionRequestFromInput_(const InputPlugWidget *,
                                             const QPoint &)));

    connect(&*inputs[i], SIGNAL(beenRightClicked(InputPlugWidget *)), this,
            SLOT(openPopup_(InputPlugWidget *)));

    connect(&*inputs[i], SIGNAL(underMouse(const InputPlugWidget *)), this,
            SLOT(mouseOverInputPlug_(const InputPlugWidget *)));
  ...
  for (unsigned int j = 0; j < _info.getOutputs().size(); j++) {
    ...
    connect(&*outputs[j], SIGNAL(underMouse(const OutputPlugWidget *)), this,
            SLOT(mouseOverOutputPlug_(const OutputPlugWidget *)));
  }

  m_properties = utils::AutoPtr<IPropertyDescription>(
      new NodeProperty(_info, inputs, dispatcher, mcr, media_path));
...
void NodeWidget::setTime(double t) { m_time = t; }

double NodeWidget::getTime() const { return m_time; }
...
void NodeWidget::openPopup_(InputPlugWidget *in) { emit openPopup(in); }
...
std::vector<OutputPlugWidget *> NodeWidget::getOutputs() const {
  ...
  for (OutputVector::const_iterator it = outputs.begin(); it != outputs.end();
       ++it) {
...
std::vector<InputPlugWidget *> NodeWidget::getInputs() const {
  ...
  for (InputVektor::const_iterator it = inputs.begin(); it != inputs.end();
       ++it) {
    temp.push_back(it->c_ptr());
  }
  return temp;
