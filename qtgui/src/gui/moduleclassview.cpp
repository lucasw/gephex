...
namespace gui {
ModuleClassView::ModuleClassView(QMenu *menu) : QObject(menu), m_menu(menu) {}
...
void ModuleClassView::moduleClassLoaded(const std::string &moduleClassName,
  ...
  std::map<std::string, QMenu *>::const_iterator it = groups.find(group);
  if (it == groups.end()) {
  ...
  connect(currentGroup, SIGNAL(triggered(QAction *)), this,
          SLOT(moduleClassSelected(QAction *)));
...
void ModuleClassView::moduleClassUnloaded(const std::string &moduleClassName) {
  ...
  std::map<std::string, QMenu *>::iterator jt = menues.find(moduleClassName);
  if (jt == menues.end())
    throw std::runtime_error("Fehler bei moduleClassUnloaded()");
...
void ModuleClassView::syncStarted() { clear(); }
