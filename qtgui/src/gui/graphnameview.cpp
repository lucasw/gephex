...
FolderItem::FolderItem(IModelControlReceiver &model, const std::string graphID,
                       const std::string &path, const std::string &displayName,
                       int mask)
    : m_model(model), m_graphID(graphID), m_path(path), m_name(displayName),
      m_mask(mask) {}

std::string FolderItem::getPath() const { return m_path; }
...
void FolderItem::newGraphSlot() {
  ...
  while (retry) {
    const std::string newName =
        AskForStringDialog::open(0, "New Graph", "Enter a name");
...
GraphItem::GraphItem(const std::string &graphID, const std::string &path,
...
void GraphItem::snapAdded() { m_numberOfSnaps++; }

// must be called when a snapshot is removed from this graph
void GraphItem::snapRemoved() { m_numberOfSnaps--; }

int GraphItem::numberOfSnaps() const { return m_numberOfSnaps; }
...
std::string GraphItem::graphID() const { return m_graphID; }

std::string GraphItem::graphName() const { return m_graphName; }
...
void GraphItem::saveGraphSlot() { m_model.saveGraph(m_graphID); }

void GraphItem::copyGraphSlot() {
  bool retry = true;
  while (retry) {
    const std::string newName =
        AskForStringDialog::open(0, "New Graph", "Enter the name of the copy");
    if (checkNamePolicy(newName)) {
...
void GraphItem::renameGraphSlot() {
  ...
  while (retry) {
    const std::string newName =
        AskForStringDialog::open(0, "New Graph", "Enter new name");
    if (checkNamePolicy(newName)) {
...
void GraphItem::killGraphSlot() { m_model.deleteGraph(m_graphID); }

void GraphItem::newSnapshotSlot() {
  std::string newName =
      AskForStringDialog::open(0, "New Snapshot", "Enter a name");
  m_model.newControlValueSet(m_graphID, newName);
}

SnapItem::SnapItem(const std::string &graphID, const std::string &snapID,
                   const std::string &graphName, const std::string &snapName,
                   IModelControlReceiver &model,
                   GraphNameViewObject &stupidObject)
    : m_graphID(graphID), m_snapID(snapID), m_graphName(graphName),
      m_snapName(snapName), m_model(model), m_stupidObject(stupidObject) {}
...
std::string SnapItem::graphID() const { return m_graphID; }

std::string SnapItem::snapID() const { return m_snapID; }

std::string SnapItem::graphName() const { return m_graphName; }

std::string SnapItem::snapName() const { return m_snapName; }
...
void SnapItem::renameSlot() {
  std::string newName =
      AskForStringDialog::open(0, "Rename Snapshot", "Enter new name");
...
void SnapItem::copySlot() {
  std::string newName =
      AskForStringDialog::open(0, "Copy Snapshot", "Enter a name for the copy");

  m_model.copyControlValueSet(m_graphID, m_snapID, newName);
}

GraphNameViewObject::GraphNameViewObject(QObject *parent, GraphNameView &view)
...
void GraphNameViewObject::undisplayProperties_() { emit undisplayProperties(); }

GraphNameView::GraphNameView(QWidget *parent, IModelControlReceiver &model,
                             IErrorReceiver &log)
    : stupidObject(0, *this), editSnap(0), renderedGraph(0), m_model(model),
      m_topItem(
          new FolderItem(model, "_", "", "Graphs",
...
QObject *GraphNameView::signalObject() { return &stupidObject; }

QWidget *GraphNameView::widget() { return m_treeView->widget(); }
 ...
 */
void extractPath(const std::string &fullPath, std::string &path,
                 std::string &last) {
...
void GraphNameView::graphExists(const std::string &graphID,
  ...
  SnapMap::const_iterator it2 = m_snaps.find(std::make_pair(graphID, snapID));
  ...
  if (it == m_graphs.end()) {
    ...
    if (parent == 0) {
      m_log.error("graphNameExists: Could not find folder '" + folderName +
                  "/'");
      return;
    }
    ...
    {
      folder = true;
      FolderItemPtr tmp(new FolderItem(
          m_model, graphID, folderName + "/" + fileName, "%" + fileName + "%"));
    ...
    } else // create a graph
    {
      folder = false;
      GraphItemPtr tmp(
          new GraphItem(graphID, folderName + "/", fileName, m_model));
  ...
  if (!folder) {
    ...
    SnapItemPtr tmp(new SnapItem(graphID, snapID, graphName, snapName, m_model,
                                 stupidObject));
...
void GraphNameView::graphNoLongerExists(const std::string &graphID,
  ...
  if (snapID != "") // graph
  {
    ...
    if (it == m_graphs.end() || it2 == m_snaps.end()) {
      m_log.error("Graph '" + graphID + ":" + snapID +
                  "' doesnt exist at "
                  "GraphNameView::graphNoLongerExists()");
      return;
    }
...
void GraphNameView::markEditGraph(const std::string &graphID,
                                  const std::string &snapID) {
  SnapMap::const_iterator it = m_snaps.find(std::make_pair(graphID, snapID));
...
bool checkNamePolicy(const std::string &name) {
  ...
  for (std::string::const_iterator it = name.begin(); it != name.end(); ++it) {
    std::string::const_iterator it2 =
        std::find(allowedCharacters.begin(), allowedCharacters.end(), *it);
