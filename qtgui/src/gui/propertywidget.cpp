...
PropertyWidget::PropertyWidget(QWidget *parent, Qt::WFlags fl, int nodeID,
                               int inIndex, int controlID,
  ...
  connect(m_view, SIGNAL(valueChanged(const utils::Buffer &)), this,
          SLOT(changeValue(const utils::Buffer &)));
...
int PropertyWidget::getControlID() const { return m_controlID; }

int PropertyWidget::getNodeID() const { return m_nodeID; }

int PropertyWidget::getInputIndex() const { return m_inputIndex; }

void PropertyWidget::setValue(const utils::Buffer &b) {
  mcr.setInputValue(m_nodeID, m_inputIndex, b);
}
void PropertyWidget::changeValue(const utils::Buffer &b) { setValue(b); }
