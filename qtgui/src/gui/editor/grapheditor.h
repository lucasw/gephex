...
public:
  ...
  GraphEditor(QWidget *parent, GraphModel &contr,
              const IModuleInfoBaseStation &,
              const utils::AutoPtr<ControlValueDispatcher> &dispatcher_,
              IModelControlReceiver &, KeyboardManager *kbManager,
...
signals:
  void createControl(const std::string &name, const std::string &type,
                     int nodeID, int inputIndex, const ParamMap &params,
                     const QPoint &);
   ...
   */
  void newEditGraph(const std::string &graphID, const std::string &snapID);
