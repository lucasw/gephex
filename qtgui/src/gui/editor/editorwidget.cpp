...
EditorWidget::EditorWidget(
    QWidget *parent, GraphModel &graphModel, IModelControlReceiver &model,
    ControlModel &controlModel, ModuleClassView &base,
    const utils::AutoPtr<ControlValueDispatcher> &dispatcher,
    IModuleStatisticsSender &mss, IModelStatusSender &msts,
    KeyboardManager *kbManager, IErrorReceiver &log,
    const std::string &media_path)

    : QSplitter(Qt::Vertical, parent) {
  GraphEditorWindow *geWindow = new GraphEditorWindow(
      this, graphModel, base, dispatcher, model, kbManager, log, media_path);
  ...
  connect(&base, SIGNAL(selectModuleClass(const std::string &)), graphEditor,
          SLOT(moduleClassSelected(const std::string &)));

  /*  connect(&extendedBase, SIGNAL(selectModuleClass(const std::string&)),
      graphEditor, SLOT(moduleClassSelected(const std::string&)));*/

  connect(graphEditor, SIGNAL(statusText(const std::string &)), this,
  ...
  connect(graphEditor, SIGNAL(undisplayProperties()), this,
          SLOT(undisplayProperties_()));

  connect(graphEditor,
          SIGNAL(newEditGraph(const std::string &, const std::string &)), this,
          SLOT(newEditGraphSlot(const std::string &, const std::string &)));

  ControlEditorWindow *ceWindow =
      new ControlEditorWindow(0, controlModel, model, dispatcher, media_path);
  ...
  connect(graphEditor,
          SIGNAL(createControl(const std::string &, const std::string &, int,
                               int, const ParamMap &, const QPoint &)),
          controlEditor,
          SLOT(selectWidgetType(const std::string &, const std::string &, int,
                                int, const ParamMap &, const QPoint &)));

  connect(controlEditor, SIGNAL(inputSelected(int, int)), graphEditor,
          SLOT(highlightInput(int, int)));
...
void EditorWidget::undisplayProperties_() { emit undisplayProperties(); }

void EditorWidget::newEditGraphSlot(const std::string &g,
                                    const std::string &s) {
