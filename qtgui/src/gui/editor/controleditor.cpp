...
ControlEditor::ControlEditor(QWidget *parent, ControlModel &cModel,
                             IModelControlReceiver &model,
                             const utils::AutoPtr<ControlValueDispatcher> &disp,
                             const std::string &media_path)
    : QWidget(parent), m_controller(&cModel), m_model(&model),
      m_factory(new TypeViewFactory(media_path)), currentNodeID(-1),
      currentInputIndex(-1), currentWidgetType(""), currentControl(0),
      m_controlValueDispatcher(disp) {}

ControlEditor::~ControlEditor() { delete m_factory; }
...
void ControlEditor::controlAdded(int controlID, const std::string &name,
  ...
  ControlWidget *cWidget =
      new ControlWidget(this, name, controlID, nodeID, inputIndex, params, con);

  if (cWidget == 0) {
    // TODO
    std::cerr << "Could not create ControlWidget at "
              << "ControlEditor::controlAdded()" << std::endl;
    return;
  }
  ...
  QObject::connect(cWidget,
                   SIGNAL(valueChanged(int, int, const utils::Buffer &)), this,
                   SLOT(controlChanged(int, int, const utils::Buffer &)));

  connect(cWidget, SIGNAL(moved(ControlWidget *, const QPoint &)), this,
          SLOT(controlWidgetMoved(ControlWidget *, const QPoint &)));

  connect(cWidget, SIGNAL(released(ControlWidget *, const QPoint &)), this,
          SLOT(controlWidgetReleased(ControlWidget *, const QPoint &)));

  connect(cWidget, SIGNAL(beenRightClicked(ControlWidget *, const QPoint &)),
          this, SLOT(openPopup(ControlWidget *, const QPoint &)));

  m_controlValueDispatcher->registerValueReceiver(nodeID, inputIndex, *cWidget);

  this->controlWidgetMoved(cWidget,
                           mapToGlobal(QPoint(clickedPos.x(), clickedPos.y())));

  // add a mouseover object to detect wether the mouse is over the
  // controlwidget

  MouseOver *mo = new MouseOver(cWidget);
  connect(mo, SIGNAL(mouseOver(QWidget *)), this,
          SLOT(mouseOverControl(QWidget *)));
...
void ControlEditor::controlMoved(int controlID, const Point &p) {
  ...
  which->move(std::max(0, p.x()), std::max(0, p.y()));
}
...
void ControlEditor::controlDeleted(int controlID) {
  ...
  m_controlValueDispatcher->unregisterValueReceiver(
      cWidget->nodeID(), cWidget->inputIndex(), *cWidget);
...
void ControlEditor::labelAdded(int labelID, const std::string &text) {
  ...
  if (lWidget == 0) {
    ...
    std::cerr << "Could not create LabelWidget at "
              << "ControlEditor::labelAdded()" << std::endl;
    return;
  }

  labels.insert(std::make_pair(labelID, lWidget));

  connect(lWidget, SIGNAL(moved(LabelWidget *, const QPoint &)), this,
          SLOT(labelWidgetMoved(LabelWidget *, const QPoint &)));

  connect(lWidget, SIGNAL(released(LabelWidget *, const QPoint &)), this,
          SLOT(labelWidgetReleased(LabelWidget *, const QPoint &)));
...
void ControlEditor::labelMoved(int labelID, const Point &p) {
  ...
  if (lewit == labels.end()) {
    // TODO
    std::cerr << "No such label at ControlEditor::labelMoved()" << std::endl;
    return;
  }
...
void ControlEditor::labelDeleted(int labelID) {
  ...
  if (lewit == labels.end()) {
    // TODO
    std::cerr << "No such label at ControlEditor::labelDeleted()" << std::endl;
...
void ControlEditor::selectWidgetType(const std::string &name,
                                     ...
                                     int inputIndex, const ParamMap &params,
                                     const QPoint &pos) {
  ...
  TypeViewFactory::TypeViewInfoList infos = m_factory->getCompatibleViews(type);
  ...
  if (infos.empty()) {
    ...
    for (TypeViewFactory::TypeViewInfoList::const_iterator i = infos.begin();
         i != infos.end(); ++i) {
    ...
    connect(popme, SIGNAL(triggered(QAction *)), this,
            SLOT(actionTriggeredSlot(QAction *)));
    popme->popup(pos);
  }
}
...
void ControlEditor::mousePressEvent(QMouseEvent *e) {
  ...
  if (e->button() == Qt::LeftButton) {
    ...
    connect(labelAction, SIGNAL(triggered()), this, SLOT(addLabelSlot()));
...
void ControlEditor::controlWidgetReleased(ControlWidget *n, const QPoint &pos) {
  ...
  try {
    m_controller->moveControl(n->controlID(), Point(p.x(), p.y()));
  } catch (std::exception &err) {
    QMessageBox::information(0, "Error", err.what());
  }
}
...
void ControlEditor::openPopup(ControlWidget *which, const QPoint &pos) {
  ...
  connect(killAction, SIGNAL(triggered()), this, SLOT(killControlSlot()));
...
void ControlEditor::labelWidgetReleased(LabelWidget *n, const QPoint &pos) {
  QPoint p = mapFromGlobal(pos);
  try {
    m_controller->moveLabel(n->id(), Point(p.x(), p.y()));
  } catch (std::exception &err) {
    QMessageBox::information(0, "Error", err.what());
  }
}
...
void ControlEditor::openLabelPopup(LabelWidget *which, const QPoint &pos) {
  ...
  currentLabel = which;
  connect(killAction, SIGNAL(triggered()), this, SLOT(killLabelSlot()));
...
void ControlEditor::addLabelSlot() {
  const std::string text =
      AskForStringDialog::open(0, "New Label", "Enter label text");
