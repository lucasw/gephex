...
class CombinedEditor : public QWidget, public IEditor {
public:
  ...
  virtual void elementsConnected(int id1, int outIndex, int id2, int inIndex);

  virtual void elementsDisconnected(int elemID, int inIndex);

  virtual void connectInputInProgress(int elemID, int inIndex, int x, int y);
  virtual void connectOutputInProgress(int elemID, int outIndex, int x, int y);
...
private:
  ...
  typedef std::map<std::pair<int, int>, std::pair<int, int>> ConnectionMap;
