...
CombinedEditor::CombinedEditor(QWidget *parent)
    : QWidget(parent, "CombinedEditor", 0), m_listener(0), m_lastID(-1) {}
...
void CombinedEditor::elementsConnected(int id1, int outIndex, int id2,
                                       int inIndex) {
  ...
  if (outIndex < 0 || outIndex >= elem1->numberOfOutputs() || inIndex < 0 ||
      inIndex >= elem2->numberOfInputs()) {
...
void CombinedEditor::elementsDisconnected(int elemID, int inIndex) {
  ConnectionMap::iterator it =
      m_connections.find(std::make_pair(elemID, inIndex));
...
void CombinedEditor::connectInputInProgress(int elemID, int inIndex, int x,
                                            int y) {}

void CombinedEditor::connectOutputInProgress(int elemID, int outIndex, int x,
                                             int y) {}
