...
GraphEditor::GraphEditor(
    QWidget *parent, GraphModel &contrl, const IModuleInfoBaseStation &_infos,
    ...
    : QWidget(parent), nodes(), connections(), selectedConnectionPair(-1, -1),
      clickedPos(QPoint(-1, -1)), currentModuleClassName(""),
      highlightedInput(0), m_controller(&contrl), infos(&_infos),
      dispatcher(_dispatcher), model(mod), m_kbManager(kbManager), m_log(log),
      m_drawmoduleinfo(false), m_property_id(-1), m_media_path(media_path),
      m_tick_count(0), m_last_highlight(0) {
...
void GraphEditor::moduleAdded(const std::string &moduleClassName, int modID) {
  ...
  NodeWidget *nWidget =
      new NodeWidget(this, 0, modID, mi, m_picmanager, dispatcher, model,
                     m_kbManager, m_log, m_media_path);
  ...
  for (std::vector<OutputPlugWidget *>::const_iterator oit = outs.begin();
       oit != outs.end(); ++oit) {
    numConnections[*oit] = 0;
  }

  if (clickedPos != QPoint(-1, -1)) {
    this->m_controller->moveModule(modID,
                                   Point(clickedPos.x(), clickedPos.y()));
  ...
  connect(nWidget, SIGNAL(clickedLeft(NodeWidget *)), this,
          SLOT(nodeWidgetClicked(NodeWidget *)));
  connect(nWidget, SIGNAL(moved(NodeWidget *, const QPoint &)), this,
          SLOT(nodeWidgetMoved(NodeWidget *, const QPoint &)));
  connect(nWidget, SIGNAL(released(NodeWidget *, const QPoint &)), this,
          SLOT(nodeWidgetReleased(NodeWidget *, const QPoint &)));

  connect(nWidget, SIGNAL(beenRightClicked(NodeWidget *, const QPoint &)), this,
          SLOT(openPopup(NodeWidget *, const QPoint &)));

  connect(nWidget, SIGNAL(redrawLine(const QPoint &, const QPoint &)), this,
          SLOT(redrawLine(const QPoint &, const QPoint &)));

  connect(nWidget,
          SIGNAL(connectionRequestFromInput(const InputPlugWidget *,
                                            const QPoint &)),
          this,
          SLOT(connectionRequestFromInput(const InputPlugWidget *,
                                          const QPoint &)));
  ...
  connect(nWidget,
          SIGNAL(connectionRequestFromOutput(const OutputPlugWidget *,
                                             const QPoint &)),
          this,
          SLOT(connectionRequestFromOutput(const OutputPlugWidget *,
                                           const QPoint &)));

  connect(nWidget, SIGNAL(openPopup(InputPlugWidget *)), this,
          SLOT(openPopup(InputPlugWidget *)));

  connect(nWidget, SIGNAL(mouseOverNode(const NodeWidget *)), this,
          SLOT(mouseOverNode(const NodeWidget *)));

  connect(nWidget, SIGNAL(mouseOverInputPlug(const InputPlugWidget *)), this,
          SLOT(mouseOverInputPlug(const InputPlugWidget *)));

  connect(nWidget, SIGNAL(mouseOverOutputPlug(const OutputPlugWidget *)), this,
          SLOT(mouseOverOutputPlug(const OutputPlugWidget *)));
...
void GraphEditor::modulesConnected(int moduleID1, int outputIndex,
  ...
  connections.insert(ConnectionMap::value_type(
      std::make_pair(moduleID2, inputIndex), connWidget));

  update();
}

void GraphEditor::modulesDisconnected(int moduleID, int inputIndex) {
  ConnectionMap::iterator cwit =
      connections.find(std::make_pair(moduleID, inputIndex));
...
void GraphEditor::moduleDeleted(int moduleID) {
  ...
  emit createControl("", "", -1, -1, std::map<std::string, std::string>(),
                     QPoint(0, 0));
...
void GraphEditor::moduleMoved(int nodeID, const Point &p) {
  ...
  nWidget->move(std::max(0, p.x()), std::max(0, p.y()));
  update();
}
...
const OutputPlugWidget *
GraphEditor::getOutputPlugByPos(const QPoint &to) const {
...
void GraphEditor::connectionRequest(const InputPlugWidget *in,
  ...
  if (in->getType() != out->getType()) {
    ...
    ConnectionMap::const_iterator it =
        connections.find(std::make_pair(module2ID, inputIndex));
...
void GraphEditor::internalsSlot() {
  ...
  caption << currentNode->moduleClassName() << ":" << currentNode->getID()
          << " Internals";
  ...
  QMessageBox::about(this, caption.str().c_str(), txt.str().c_str());
}
...
void GraphEditor::connectToControlSlot() {
  ...
  emit createControl(name, currentInput->getType(), currentInput->getID(),
                     currentInput->getIndex(), currentInput->getParams(),
...
void GraphEditor::disconnectSlot() {
  model.disconnectModules(currentInput->getID(), currentInput->getIndex());
}

void GraphEditor::killConnectionSlot() {
  try {
    ConnectionMap::const_iterator it = connections.find(selectedConnectionPair);
    if (it == connections.end()) {
...
void GraphEditor::paintEvent(QPaintEvent * /*e*/) {
  ...
  if (m_drawmoduleinfo) {
    for (std::map<int, NodeWidget *>::const_iterator jt = nodes.begin();
      ...
      mainPainter.eraseRect(nWidget->pos().x(), nWidget->pos().y() - 10, 40,
                            10);

      mainPainter.drawText(nWidget->pos().x(), nWidget->pos().y() - 5,
                           msg.str().c_str());
    }
  }

  // Draw current connection attempt
  if (m_currentFrom != QPoint(0, 0) && m_currentTo != QPoint(0, 0)) {
...
void GraphEditor::timer_fired() {
  if (highlightedInput && (m_tick_count - m_last_highlight) * TIMER_MS > 1000) {
    highlightedInput->removeHighlight();
    highlightedInput = 0;
  }
...
void GraphEditor::incConnectionCount(PlugWidget *plug) {
  std::map<const PlugWidget *, int>::const_iterator it =
      numConnections.find(plug);
