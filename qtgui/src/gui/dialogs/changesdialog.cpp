 ...
 */
ChangesDialog::ChangesDialog(QWidget *parent, bool modal, Qt::WFlags fl)
    : QDialog(parent, fl) {
  setModal(modal);
  setupUi(this);
  ...
  changes_view->setHtml(
      ...
      "</ul>\n"
      "<ul><li>fixed bugs #78, #79, #80, #81, #82, #83, #84, #85, #86, #87, "
      "#88, "
      "#89, #90, #92  #96, #97, #100, #102, #103, #112, #113, #114, #115, "
      "#116, "
      "#125, 126\n"
      "</ul>\n"
      "<p>\n"
      ...
      "<ul><li>fixed bugs #67, #68, #69, #70, #71, #72, #74, #75, "
      "#77</li></ul>\n"
      "<p>\n"
      "\n"
      "<hr/>"
      ...
      "certain conditions\n"
      "<li>image output      : unified xshm and ximage driver, added Xv "
      "support (uses hardware scaling if supported)\n"
      "<li>rotozoom module   : added mirror-mode\n"
      "<li>tunnel module     : fixed cylindrical mapping, removed some "
      "artifacts\n"
      "</ul></ul>\n"
      "<p>\n"
      "<ul><li>added command line parameter support to engine and gui (now all "
      "parameters in gephex.conf can be overriden on the command line) \n"
      "<li>replaced most inline assembler code with external nasm assembler "
      "files\n"
      "</ul>\n"
      "<p>\n"
      ...
      "install dir to <code "
      "class=var>$prefix</code>/lib/gephex-0.4/(modules|types)\n"
      "</ul>\n"
      "<p>\n"
      "\n"
      "<hr/>"
      "\n"
      "<h3>0.4:</h3>\n"
      "<ul><li>added win32 build system files (.dsp, .dsw, .mak, .dep) to the "
      "source dist. From now on it is possible to build the windows binaries "
      "from the source dist (see README.win32).\n"
      "</ul>\n"
      "<ul><li>vloopback support in image output module (can be used to chain "
      "gephex with another application that supports v4l, like effecTV or "
      "camstream)\n"
      "</ul>\n"
      "<ul><li>support for capture devices via directshow, this should cover "
      "most webcams and tv cards (capturemodule)\n"
      "</ul>\n"
      "<ul><li>improved usability of the graph editor and control editor\n"
      "<ul><li>the control editor now supports text labels to document the "
      "controls of an effect-graph\n"
      "<li>when the mouse is over a control widget, the controlled input is "
      "highlighted in the graph editor\n"
      "<li>improved (or rather fixed) the selection of connections between "
      "modules (graph editor)\n"
      "</ul></ul>\n"
      "<ul><li>new and improved plugins\n"
      "<ul><li>greatly improved performance of gradnormmodule, added different "
      "edge operators\n"
      "<li>new function parse module with 4 number inputs which can interpret "
      "arithmetic expressions (more flexible than the calculon module)\n"
      "<li>alsa driver for midi input\n"
      "<li>reactivated linux native joystick driver (now you don't need SDL to "
      "use the joystick on linux) \n"
      "<li>rewrote and activated the direct show based video driver in the "
      "frbinmodule (image source)\n"
      "<li>block size in pixelizermodule can now be chosen independently for x "
      "and y direction\n"
      ...
      "<ul><li>fixed bug with select (#24) which also fixed a bug with the "
      "linux joystick driver (#3) \n"
      "<li>fixed saturation bug (#18) in the gradnormmodule \n"
      "<li>fixed compilation issue with alsa 1.0 (bug #20) \n"
      "<li>fixed bug (#25) in renderer which caused skipped updates in some "
      "situations \n"
      "<li>fixed bug (#29) of the install-exec-hook in the data dir\n"
      "<li>fixed \"jumps\" of the signal generator when frequency or signal "
      "type changes (bug #35)\n"
      "<li>fixed bug (#36) which caused wrong values on inputs after "
      "disconnect\n"
      "</ul></ul>\n"
      "\n"
      "<hr/>"
      ...
      "</ul>\n"
      "\n"
      "<ul><li>changed image source: removed automatic seeking and changed "
      "from an [0,1] position input to [0, num_frames-1] position input\n"
      "</ul>\n"
      "\n"
      "<ul><li>frboutmodule (unix only): ximage, xshm and gl output driver now "
      "use the DISPLAY environment variable if their display-property is set "
      "to \"default\"\n"
      "</ul>\n"
      "\n"
      ...
      "</ul>\n"
      "\n"
      "<ul><li>added the ifs (iterated function system) effects to the win32 "
      "version\n"
      "</ul>\n"
      "\n"
      ...
      "</ul>\n"
      "\n"
      "<ul><li>added wrapper script for better installation (example graphs "
      "are installed under linux now, too) \n"
      "</ul>\n"
      "\n"
      ...
      "</ul></ul>\n"
      "\n"
      "<ul><li>changed from debug to release target for engine and gui on "
      "win32 (unix version already was non-debug per default)\n"
      "</ul>\n"
      "\n"
      "<hr/>\n"
      "\n"
      ...
      "<ul><li>fixed crash when deleting module that is connected to a patched "
      "input\n"
      ...
      "</ul></ul>\n"
      "\n"
      "<ul><li>gui and engine now run independent, when one app crashes the "
      "other one continues \n"
      "</ul>\n"
      "\n"
      ...
      "</ul>\n"
      "\n"
      "<ul><li>improved startup speed (loading the plugins took very long "
      "before)\n"
      "</ul>\n"
      "\n"
      "<ul><li>new modules:\n"
      ...
      "</ul>\n"
      "\n"
      "<ul><li>more bug fixes and small improvements in engine, gui and "
      "modules\n"
      "</ul>\n"
      "\n"
      "<hr/>\n"
      ...
      "<h3>0.0.4alpha2</h3>\n"
      "\n"
      "<ul><li>audio type and some audio modules (beat detection works, but "
      ...
      "</ul>\n"
      "\n"
      "<ul><li>merged several modules for better usability: (please see README "
      "if you want to convert old graphs)\n"
      "<ul><li>all framebuffer outputs are merged into frboutmodule (the same "
      "module for linux and windows, this way the graphs are more portable) "
      "[note that some outputs like glout and sdlout are commented out for the "
      "moment due to their bad performance] \n"
      "<li>bmpmodule, avifilemodule, libmpeg3loader, videomodule, "
      "directshowmodule are merged into frbinmodule (again only one module for "
      "all platforms) \n"
      "<li>addmodule and submodule have been merged into arithmodule (which "
      "now can do a lot more than just addition and subtraction) \n"
      "<li>imgmulmodule, imgaddmodule, imgsubmodule, invertmodule have been "
      "fixed and merged into imgarithmodule \n"
      "</ul></ul>\n"
      "\n"
      "<ul><li>all framebuffer output drivers now support control over "
      ...
      "<li>audio(in|out)module : in and output, supports alsa and OSS on posix "
      ...
      "</ul>\n"
      "\n"
      "<ul><li>removed output size from rotary zoomer, the size of the input "
      ...
      "<h3>0.0.4alpha1</h3>\n"
      "\n"
      "<ul><li>NumberType changed from unsigned int (32 bit) to IEEE 64 bit "
      "floating point value. This changes nearly all effects. \n"
      "<li>Many bug fixes\n"
      "</ul>\n"
      "\n"
      "<ul><li>Color-format and framebuffer orientation fixed to BGRA top down "
      "(i.e. first line in memory is upper most line)\n"
      "</ul>\n"
      "\n"
      ...
      "</ul>\n"
      "\n"
      "<ul><li>Better control and property widgets (refactored them to use "
      "common code)\n"
      "</ul>\n"
      "\n"
      "<hr/>\n"
      ...
      "</ul>\n"
      "\n"
      "<ul><li>Reworked network code (support for unix domain sockets + named "
      "pipes)\n"
      "</ul>\n"
      "<hr/>\n"
      "\n");
