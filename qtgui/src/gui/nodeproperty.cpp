...
NodeProperty::NodeProperty(
    const ModuleInfo &_info, std::vector<utils::AutoPtr<InputPlugWidget>> &ins,
    const utils::AutoPtr<ControlValueDispatcher> &dispatcher,
    IModelControlReceiver &mcr, const std::string &media_path)
...
NodeProperty::~NodeProperty() { delete factory; }

std::list<PropertyEntry> NodeProperty::getEntries() const { return m_entries; }

void NodeProperty::addProperty(
    InputPlugWidget &in,
    const utils::AutoPtr<ControlValueDispatcher> &dispatcher,
  ...
  try {
    ...
    ctors.push_back(new PropertyWidgetConstr(
        con, current.params, nodeID, inputIndex, -1, dispatcher, model));
