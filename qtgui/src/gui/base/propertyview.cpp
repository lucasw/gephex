...
PropertyView::PropertyView(QWidget *parent) : QWidget(parent), table(0) {
  m_layout = new QHBoxLayout(this);
}

PropertyView::~PropertyView() { deleteTable(); }

void PropertyView::deleteTable() {
  if (table != 0) {
    for (std::list<std::pair<const IWidgetConstructor *, QWidget *>>::
             const_iterator it = widgetCtors.begin();
...
void PropertyView::displayProperties(const IPropertyDescription &desc) {
  ...
  for (std::list<PropertyEntry>::const_iterator it = entries.begin();
    ...
    table->setVerticalHeaderItem(
        rowIndex, new QTableWidgetItem(current.getName().c_str()));

    const std::list<const IWidgetConstructor *> &wCtors =
        current.getWidgetCtors();
    int colIndex = 0;
    for (std::list<const IWidgetConstructor *>::const_iterator it2 =
             wCtors.begin();
...
void PropertyView::undisplayProperties() { this->deleteTable(); }
