...
public:
  ColumnTextChangeListenerImpl(QTreeWidgetItem *item) : m_item(item) {}
...
TreeViewImpl::TreeViewImpl(QWidget *parent, const std::string &name,
                           ...
                           TreeView *treeview)
    : QTreeWidget(parent), activeItem(0), m_treeview(treeview) {
  ...
  connect(this, SIGNAL(itemActivated(QTreeWidgetItem *, int)), this,
          SLOT(itemActivatedSlot(QTreeWidgetItem *, int)));
}

TreeViewImpl::~TreeViewImpl() { delete m_treeview; }
...
void TreeViewImpl::insertItem(TreeViewItem &item, TreeViewItem *parent) {
  ...
  TextChangeListenerPtr newListener(
      new ColumnTextChangeListenerImpl(&*newItem));
...
void TreeViewImpl::itemClickedSlot(QTreeWidgetItem *item, int column) {
  if (!item)
    return;
...
void TreeViewImpl::itemActivatedSlot(QTreeWidgetItem *item, int column) {
  if (!item)
    return;
...
QWidget *TreeView::widget() { return m_impl; }

void TreeView::insertItem(TreeViewItem &item, TreeViewItem *parent) {
  m_impl->insertItem(item, parent);
}

void TreeView::removeItem(TreeViewItem &item) { m_impl->removeItem(item); }
} // namespace gui
