...
T combine_reduce(InputIterator1 first1, InputIterator1 last1,
                 ...
                 BinaryFunction1 binary_op_r, BinaryFunction2 binary_op_c) {
...
static bool binary_and(bool b1, bool b2) { return b1 && b2; }

// case sensitive compare function
static bool char_equals(char t1, char t2) { return t1 == t2; }

// TODO: workaround for mingw 3.4.2
static char my_tolower(char c) { return tolower(c); }
...
bool is_prefix(std::string prefix, std::string s) {
  ...
  std::transform(prefix.begin(), prefix.end(), prefix.begin(), my_tolower);

  std::transform(s.begin(), s.end(), s.begin(), my_tolower);
#endif

  return prefix.length() <= s.length() &&
         combine_reduce(s.begin(), s.end(), prefix.begin(), prefix.end(), true,
                        binary_and, char_equals);
  /*
...
static std::string path_to_rel(const std::string &path,
  ...
  std::list<std::string>::const_iterator it = std::find_if(
      dirs.begin(), dirs.end(), std::bind2nd(std::ptr_fun(is_prefix), path));
...
FileStringView::FileStringView(QWidget *parent, const ParamMap &params,
...
void FileStringView::selectFile() {
  ...
  QStringList files = QFileDialog::getOpenFileNames(
      this, "choose files", path.c_str(), m_fullMask.c_str());
  ...
  if (name.length() != 0) {
    utils::Buffer b(reinterpret_cast<const unsigned char *>(name.c_str()),
                    name.length() + 1);
