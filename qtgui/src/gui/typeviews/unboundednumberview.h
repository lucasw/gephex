...
public:
  UnboundedNumberView(QWidget *parent, const ParamMap &params)
    ...
    std::string display_format = sr.getStringValue("display_format", "auto");
...
public slots:
  void lineeditChanged() {
    ...
    utils::Buffer b = utils::Buffer(
        reinterpret_cast<const unsigned char *>(text), strlen(text) + 1);
...
class UnboundedNumberViewConstructor : public TypeViewConstructor {
public:
