...
public:
  CheckBoxNumberView(QWidget *parent, const ParamMap &params)
    ...
    try {
      ...
      for (std::string token = tok.next(","); token != "";
           token = tok.next(","), ++m_num_buttons) {
    ...
    /*
       ...
       os << "width     = " << grp->width() << "\t height     = " <<
       grp->height() << "\n"; os << "minWidth  = " << min.width()      << "\t
       minHeight  = " << min.height() << "\n"; os << "maxWidth  = " <<
       max.width()      << "\t maxHeight  = " << max.height() << "\n"; os <<
       "prefWidth = " << pref.width()     << "\t prefHeight = " << pref.height()
       << "\n"; QMessageBox::information( 0, "Info", os.str().c_str() );*/
    m_layout->addWidget(grp);
    connect(m_group, SIGNAL(buttonClicked(int)), this, SLOT(boxToggled(int)));
    grp->show();
  }
...
private slots:
  void boxToggled(int index) {
    ...
    utils::Buffer b(reinterpret_cast<const unsigned char *>(os.str().c_str()),
                    os.str().length() + 1);
    emit valueChanged(b);
  }
